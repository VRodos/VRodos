...
Desktop compiled scenes use schema v2 of `assets/desktop-performance-profiles.json`. The Custom tab owns shared artistic intent and the exact fixed-build quality; Low/Medium/High store only bounded performance overrides and use `Default`/`Modified` state. `buildMode=custom` publishes one `desktop-custom` derivative and does not load hardware selection. `buildMode=adaptive` publishes all three tiers and selects one before A-Frame, optional chunks, or GLBs load. Do not extend these controls to headset, PC-rendered-VR, mobile, or touch targets. Low/Medium clouds remain available only when Custom/High authored clouds on, and both disable cloud light shafts.
...
- Desktop compilation owns queued `desktop-custom`, `desktop-low`, `desktop-medium`, and `desktop-high` derivative identities. Custom-only builds generate just `desktop-custom`; adaptive builds generate Low/Medium/High. Low/Medium use KTX2 plus safe Draco and may simplify visual geometry; Custom/High use original textures and geometry plus safe Draco. Never simplify walkable/collision assets or GLBs with skins/morph targets.
...
- Keep dashboard-selected `safe-draco`/`safe-meshopt` substitution per-asset and opt-in. Adaptive desktop profile derivatives are a separate compiler-owned family and require the generated client decoder paths plus profile/browser parity checks.
