...
Desktop compiled scenes use schema v2 of `assets/desktop-performance-profiles.json`. The Custom tab owns shared artistic intent and the exact fixed-build quality; Low/Medium/High store only bounded performance overrides and use `Default`/`Modified` state. `buildMode=custom` publishes one `web-high` derivative and does not load hardware selection. `buildMode=adaptive` publishes Web Low/Medium/High and selects one before A-Frame, optional chunks, or GLBs load. Headset and PC-rendered-VR use the same derivative family without consuming the desktop performance-control contract. Low/Medium clouds remain available only when Custom/High authored clouds on, and both disable cloud light shafts.
...
- Activation of `vrodos_asset3d_glb` owns the versioned `web-high` -> `web-medium` -> `web-low` background family; import, status, dashboard, batch, and compilation must all join the same immutable per-asset job through `ensure_derivative()`. Custom/High and PC-rendered-VR use Web High (4096px), Desktop Medium uses Web Medium (2048px), and Desktop Low uses Web Low (1024px). Standalone headset defaults to Web Low but permits build-wide Medium/High through `vrHeadsetAssetQuality`; preparation and publication must agree on the selected recipe and texture cap. All use KTX2 plus safe Draco; Low/Medium may simplify eligible visual decoration. Web High preserves geometry, and every profile preserves walkable/collision assets and GLBs with skins or morph targets.
