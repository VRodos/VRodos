...
- Atmosphere visual assembly lives in `assets/js/runtime/master/vrodos_atmosphere_visuals.js`: sky/stars/Moon, cloud sun disk, sun/haze sprites, direct-sky calibration/reveal, and legacy environment handoff. `VRODOSMaster.AtmosphereVisuals.create` receives explicit lighting/cloud/shadow/host interfaces. Quality profiles retains configuration, precomputed resource creation, scene state, lifecycle entry points, and orchestration; visual callbacks from lighting/shadow resolve after assembly.
