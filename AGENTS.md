...
- Lights-only gradient sky creation, preset updates, and removal live in `assets/js/runtime/master/vrodos_gradient_sky.js`, exposed through the required `VRODOSMaster.GradientSky` API. Quality profiles retains scene-owned state and policy/cleanup call sites.
- Sun sprite/haze texture generation and horizon sun presentation presets live in `assets/js/runtime/master/vrodos_sun_sprite.js`. Atmosphere visuals receives the required `VRODOSMaster.SunSprite` API through assembly; texture caches and disposal ownership remain on the existing scene component.
