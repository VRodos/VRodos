...
- Sun sprite/haze texture generation and horizon sun presentation presets live in `assets/js/runtime/master/vrodos_sun_sprite.js`. Quality profiles uses the required `VRODOSMaster.SunSprite` API; per-scene texture caches, sprite creation, and disposal remain at their existing boundaries.
