...
### Headset filtering and static-work follow-up — 2026-09-17

The visible Quest scene used a fixed sun, no day/night cycle, a static HDR reflection source, and 54 tracked reflection materials. Of 49 unique scene textures inspected, 48 used 16x anisotropic filtering and one used 1x. In a five-second timing sample covering 140 XR frames, `updatePmndrsHorizonSun()` took 124.5 ms total (0.89 ms per call), `updateReflectionEnvironmentIntensity()` took 32.9 ms (0.24 ms per call), and `updateAdaptiveShadowFit(false)` took 4.7 ms (0.03 ms per call). `getPmndrsAtmosphereConfig()` was called 420 times, costing 28.4 ms total; those calls overlap the enclosing sun/reflection timings. The shadow-map update count did not change. These are CPU-side wall timings, not isolated GPU cost.

A temporary 16x/8x/16x/8x/16x texture-filtering trial retained the native XR framebuffer, MSAA, and scene materials. Head direction and visible geometry changed substantially between windows (mean stereo triangles 1.21–3.01 million), so the raw frame intervals cannot establish a filtering benefit. Only 15 of 212 sampled 8x frames found close 16x matches under the chosen pose and workload thresholds; that is insufficient for a production FPS claim. The trial restored all textures to 16x. The runtime source now caps standalone headset textures at 8x for an explicit in-headset visual trial; no eye-resolution change was made. An 8x override was also applied to the open published Quest tab for immediate visual review, and resets on reload. The repository change requires deployment and scene recompilation to persist.

Sky configuration is mostly fixed in this headset scene, but the Takram sky material's `onBeforeRender()` copies each eye camera's inverse projection and world matrix every draw. That camera-dependent update must remain. `updatePmndrsHorizonSun()` also handles pose-dependent sun occlusion and navigation yaw. A future CPU cleanup should separate fixed configuration/light setup from those pose-dependent updates rather than freezing the whole atmosphere component. Its measured cost is small relative to the 30–45 ms frame intervals.

After walking with the live 8x override, the user reported no visible change or unusual artifact. A requested held-view 8x/16x/8x comparison then recorded seven seconds per condition. The raw mean intervals were 47.00/45.82/43.97 ms, while average submitted stereo geometry also fell from 2.77/2.58/2.41 million triangles and head pose drifted across the windows. Matching frames within 2 degrees, 3 cm, 2% triangle count, and two draws gave 116 of 153 middle-window frames, with the 8x match about 1.6 ms slower on average; repeated frames and remaining time/pose differences prevent a causal filtering estimate. Thus the visual trial passed for this scene, but 8x has **no demonstrated FPS gain**. The live Quest tab was returned to 8x after the comparison. The raw per-frame record is `quest-aniso-stationary.json` in the same external audit folder.

The next source change avoids fixed Takram sky/stars/Moon setup on each standalone headset frame once resources are ready. The fast path still checks camera-dependent sun occlusion and synchronizes presented shadow-light transforms; a yaw, presentation, settings, resource, cloud, or day/night change uses the full path. With static HDR and no clouds or day/night cycle, reflection intensity updates once and repeats only when the environment, material collection, or scene settings change. The native sky shader's per-eye camera matrices still update while rendering. Runtime tests and bundle checks passed. Do not attribute an FPS gain to this change without a matched view.

After deployment and scene reload, the visible Quest XR session confirmed the new atmosphere fast path and 8x filtering on all 48 inspected scene textures. A 20-second unattended light-view sample held 110 draws and 0.37 million submitted stereo triangles: mean 22.26 ms, p95 26.1 ms, with no shadow-map updates. A five-second CPU probe at that view found 225 sun-helper calls averaging 0.33 ms each and no repeated HDR intensity updates. The earlier sun-helper sample averaged 0.89 ms and HDR intensity 0.24 ms, but those measurements used different views/session conditions. They show the intended static work is absent, not a measured FPS gain.

For a dense-view diagnostic, the sampler temporarily moved the authored world to the earlier temple navigation position, scanned view yaw, and restored the original navigation position/yaw afterward. The heaviest angle submitted 3.88 million triangles and 246 draws; a 15-second unattended sample averaged 62.22 ms (p95 67.7 ms), again with zero shadow-map updates. This angle, projected pixel coverage, device state, and set-down headset condition differ from the earlier worn dense-view sample, so the two intervals are not a before/after comparison. The dense view remains expensive and the GPU reported 99% busy during inspection. The Quest thermal service reported status 0; that status alone does not prove identical clocks or thermal history. The headset wake keeper was paused for both timing captures and restarted afterward.
