...
- Each frame syncs camera, active Takram atmosphere textures, sun direction, world-to-ECEF matrix, correct-altitude mode, authored coverage, effective coverage diagnostics, quality profile, style layer profile/signature, wind velocity, Horizon cloud haze ownership, and local cloud textures.
...
- The default style applies Takram's documented cloud-layer set. Non-default styles intentionally change conservative layer fields plus weather tiling/repeat values so `scattered`, `broken`, `overcast`, and `storm` produce visibly different cloud character without exposing raw scattering or raymarch controls.
