...
Camera-anchored prompts should use an explicit vertical policy. CEFR, immersive assessment, and image/text POI panels should use camera-relative `centerAtEyeLevel: true`, no `anchorElement`, and a short initial `anchorRefreshFrames` window so they open at readable/controller distance in front of the user, then stay fixed while the panel is open. CEFR can remain physically compact through panel width/height and a small vertical offset, not top-aligned below eye level. CEFR must wait for `vrodos-scene-loader-ready` / `vrodos-scene-loader.isReady` so blocking assets and sky/runtime reveal readiness finish first, but it should lock locomotion while waiting in immersive XR and open immediately once that readiness gate is satisfied.

Controller rays remain A-Frame-owned. `window.VRODOSControllerRayReadiness` is the shared startup gate used by navigation and spatial UI: per hand it waits for a physical tracked-pointer WebXR input source, A-Frame controller/model readiness, a non-empty raycaster, and several consecutive stable frames after XR/controller changes. Until that gate is ready, navigation hides stale controller line visuals and spatial UI keeps the panel visible but suppresses ray trimming, hit dots, and PMNDRS controller pointer clicks. Once ready, the runtime uses the active A-Frame controller raycaster as the PMNDRS pointer source, captures and restores controller line state only for stable controller-owned line visuals, trims line geometry to the dialog hit distance without changing its angle, clamps the active controller raycaster `far` distance to the same hit, shows a small hit dot anywhere the ray crosses the dialog surface, upgrades that dot to a larger actionable color over selectable controls, and restores the normal raycaster state on close. Diagnostics for the gate are exposed at `window.__vrodosLastControllerRayReadiness`. Do not create or rewrite controller `line` geometry manually, retarget A-Frame raycasters to `.vrodos-overlay-hit-target`, use detached PMNDRS/helper line geometry as controller truth, hide stable tracked/controller-present line meshes, strip controller component/visibility attributes during startup, or add invisible A-Frame hit planes for PMNDRS panels.

Navigation and spatial UI should not carry their own WebXR input-source or component-stack readiness fallbacks. If the shared readiness helper is unavailable, they should fail closed and report `missing-controller-ray-readiness-helper` instead of guessing controller readiness locally.

Controller discovery for runtime click/pointer bridges should target the generated controller entities (`#oculusLeft`, `#oculusRight`) and their authored `laser-controls` attribute. The shared readiness helper may inspect A-Frame's installed controller components for hand/model diagnostics, but runtime bridge selectors should not fan out across separate `meta-touch-controls` or `oculus-touch-controls` selectors beside `laser-controls`.

The Quest Browser controller startup regression fixed on 2026-06-22 was caused by showing/using controller ray state before A-Frame's controller pose and controller model were stable. The failed workarounds were manual `line` injection, helper target-ray visuals, raycaster retargeting, and switching pointer bridges during unstable frames. Future changes must preserve the shared readiness gate and the spatial UI stable-ray fallback: after a hand has produced a valid non-default A-Frame ray, a short readiness drop for that same hand must not detach the PMNDRS pointer while the raycaster ray is still valid. Otherwise the visible ray can look correct but pass through CEFR/assessment dialog controls, as happened with the left controller.
...
14. Test CEFR and at least one assessment dialog with both left and right controllers. Each hand's visible A-Frame ray must produce the modal surface dot and hit actionable controls; one hand passing through the dialog while the other works is a regression.
15. Enter immersive VR while holding controllers normally, and again with controllers down/asleep before picking them up. Incorrect steep startup rays should not be shown; rays should appear only after the readiness gate and should not change angle unexpectedly.
16. Open a Pair assessment and confirm selecting a source/target updates button state in place without flashing the whole dialog.
17. Confirm long assessment titles wrap to at most two header lines, ellipsize beyond that, and never overlap the close button.
18. Exit immersive VR through the Quest Browser/Oculus system UI, not only desktop `Esc`, and confirm desktop returns without fisheye projection, clicks work for video/assessment/CEFR/POI objects, and `window.__vrodosLastXrExitRestoreDiagnostics` reports inline mode with interaction unlocked.
...
- `CloudsEffect.skipRendering` stays `true` when AerialPerspectiveEffect is present, matching Takram's documented route: CloudsEffect renders its internal cloud buffers, skips direct color-buffer composition, and routes its atmosphere overlay into `AerialPerspectiveEffect`. Horizon keeps direct `SkyMaterial.shadowLength` routing disabled by default because those screen-space shadow buffers can reveal large geometric footprints on the sky during day/night motion; high/ultra desktop light shafts route through masked AerialPerspective instead.
- Horizon normally constrains `AerialPerspectiveEffect` to cloud overlay and atmosphere transmittance. When high/ultra desktop cloud light shafts are eligible, VRodos enables Aerial `sky`, `sunLight`, and `skyLight` only with a shared `NormalPass` plus Takram `LightingMaskPass` on reserved layer `30`, so authored PBR/media meshes remain lit by Takram/VRodos light sources while sky pixels receive the demo-style post-process shafting.
- Each frame syncs camera, active Takram atmosphere textures, sun direction, world-to-ECEF matrix, correct-altitude mode, authored coverage, effective coverage diagnostics, quality profile, normalized style layer profile/signature, wind velocity, Horizon cloud haze ownership, local cloud textures, weather-UV projection state, masked Aerial lighting state, normal-buffer readiness, lighting-mask readiness, and profile validation diagnostics.
- `pmndrsCloudsCoverage` remains authored cloudiness `0..1`. Horizon maps authored values through one shared `scale + bias + max` response before assigning Takram `CloudsEffect.coverage`; diagnostics expose authored coverage, effective coverage, shared coverage-mapper status, coverage-mapper signature, requested layer signature, applied layer signature, and profile fallback status.
- The default style applies Takram's documented cloud-layer set. Non-default styles intentionally change only audited safe `CloudLayer` fields: `altitude`, `height`, `densityScale`, `shapeAmount`, `shapeDetailAmount`, and optionally `shadow`. Style presets must not alter `densityProfile`, `coverageFilterWidth`, `weatherExponent`, weather repeat/offset, shape repeat, turbulence, scattering, or raymarch internals until demo-like parity is stable.
- Non-geospatial desktop Horizon clouds enable a VRodos shader define, `VRODOS_LOCAL_HORIZON_WEATHER_UV`, on both Takram cloud and shadow materials. The generated `vrodos-takram-clouds.bundle.js` patches Takram's `getGlobeUv()` so local weather samples use a tangent-plane projection instead of the documented cube-sphere local-weather UV path that can expose seams in local scenes. Geospatial/global scenes keep Takram's native `cube-sphere` mode. Diagnostics report `cloudWeatherUvMode`, `cloudWeatherUvPatchApplied`, and `cloudWorldToEcefFrame`.
...
- High and ultra desktop cloud profiles enable Takram `CloudsEffect.lightShafts` automatically only when clouds are active, masked AerialPerspective is present, the sun is above the horizon, effective coverage is high enough, and the renderer is non-XR desktop. Low and medium keep light shafts disabled for lower GPU cost; `?vrodos_debug_disable_pmndrs_cloud_light_shafts=1` remains available for troubleshooting.
