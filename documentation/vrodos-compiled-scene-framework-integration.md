...
- Each frame syncs camera, active Takram atmosphere textures, sun direction, world-to-ECEF matrix, correct-altitude mode, authored coverage, effective coverage diagnostics, quality profile, normalized style layer profile/signature, wind velocity, Horizon cloud haze ownership, local cloud textures, masked Aerial lighting state, normal-buffer readiness, lighting-mask readiness, and profile validation diagnostics.
- `pmndrsCloudsCoverage` remains authored cloudiness `0..1`. Horizon maps authored values through a simple style-specific `scale + bias + max` before assigning Takram `CloudsEffect.coverage`; diagnostics expose authored coverage, effective coverage, coverage-mapper signature, requested layer signature, applied layer signature, and profile fallback status.
