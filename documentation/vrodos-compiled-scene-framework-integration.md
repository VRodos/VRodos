...
- Each frame syncs camera, active Takram atmosphere textures, sun direction, world-to-ECEF matrix, correct-altitude mode, authored coverage, effective coverage diagnostics, quality profile, style layer profile, wind velocity, Horizon cloud haze ownership, and local cloud textures.
...
- Horizon scenes keep Takram default `coverageFilterWidth`, `weatherExponent`, and `localWeatherRepeat`. The default style applies Takram's documented cloud-layer set; non-default styles adjust only conservative layer fields such as altitude, height, density, and shape amount. VRodos does not alter weather-mask sharpness/repetition controls because local testing showed hard weather-cell silhouettes when they were pushed away from defaults.
