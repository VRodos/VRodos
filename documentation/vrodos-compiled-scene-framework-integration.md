...
- Each frame syncs camera, active Takram atmosphere textures, sun direction, world-to-ECEF matrix, correct-altitude mode, authored coverage, effective coverage diagnostics, quality profile, style layer profile/signature, wind velocity, Horizon cloud haze ownership, local cloud textures, masked Aerial lighting state, normal-buffer readiness, lighting-mask readiness, and weather seam mitigation diagnostics.
- `pmndrsCloudsCoverage` remains authored cloudiness `0..1`. Horizon maps authored values into a style-specific Takram `CloudsEffect.coverage` range because cloud-layer density math can saturate near `1 - coverageFilterWidth`; diagnostics expose authored coverage, effective coverage, coverage-mapper signature, requested layer signature, and applied layer signature.
- The default style applies Takram's documented cloud-layer set. Non-default styles intentionally change conservative layer fields plus weather tiling/repeat values so `scattered`, `broken`, `overcast`, and `storm` produce visibly different cloud character without exposing raw scattering or raymarch controls. Dense Horizon styles also apply deterministic local-weather offsets/repeats and softened near storm scud to reduce Takram cube-sphere weather seam/block artifacts.
