...
- Each frame syncs camera, active Takram atmosphere textures, sun direction, world-to-ECEF matrix, correct-altitude mode, authored coverage, effective coverage diagnostics, quality profile, normalized style layer profile/signature, wind velocity, Horizon cloud haze ownership, local cloud textures, weather-UV projection state, masked Aerial lighting state, normal-buffer ownership/readiness, real shadow-length buffer and shader-define readiness, lighting-mask readiness, and profile validation diagnostics.
