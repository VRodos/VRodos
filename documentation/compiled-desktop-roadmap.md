...
- Custom only prepares, publishes, and loads `web-high`, preserving artistic intent with conservative geometry simplification while capping pathological textures at 4096px and compressing them to KTX2. It does not include the hardware probe, adaptive query override, or runtime recommendation path.

Low inherits the previous Medium preset: Web Medium 2048px KTX2 textures, a 192 MiB scene target, budgeted visual geometry (up to 80%), a 3.7 MP/DPR 1 budget, static Medium shadows, Medium SMAA, Balanced atmosphere, and no clouds by default. Medium uses Web High 4096px KTX2 textures, a 256 MiB target, conservatively simplified geometry, a 5 MP/DPR 1.25 budget, High SMAA, Quality atmosphere, and Low-quality clouds without shafts. High is the high-fidelity preset: Web High 4096px textures without a scene texture-memory target, conservatively simplified geometry, DPR up to 2, high dynamic shadows, Ultra SMAA and High clouds (Ultra selectable), Cinematic atmosphere, and enhanced reflections with 256px slowly updated scene probes when that reflection source is authored. Authored artistic settings and effect toggles remain shared; Custom keeps the exact authored settings. Collision/navigation assets plus skinned or morph-target GLBs are never simplified.

Adaptive Low/Medium compositor eligibility follows the selected profile's enabled effects, not the old High-only render-quality gate. Clouds and the compositor share that quality decision. Saved default values follow the current preset on compilation, while explicit author modifications remain overrides. Recompile published clients after changing these presets; existing Web Medium/High derivatives are reused, and standalone headset Web Low remains 1024px.
