...
- “VR Headset Full” exposes project-build GLB object quality through `vrHeadsetAssetQuality`: Low (default, `web-low`, 1024px), Medium (`web-medium`, 2048px), or High (`web-high`, 4096px, fine geometry detail). The selected scene remembers the choice; the compile request applies it to every scene in the project. All levels apply guarded eligible decoration simplification and all collision/protected-geometry rules. High is optimized quality, not original-source quality, and requires headset GPU/memory and visual validation. This choice does not change headset renderer, lighting, or post-FX policy.
...
- With a fixed sun and no clouds or day/night cycle, the headset atmosphere tick reuses the configured Takram sky until scene settings, resources, presentation mode, or virtual yaw change. Camera-dependent sun occlusion and presented shadow-light transforms still update while walking. Static headset HDR reflection intensity likewise refreshes when the environment, materials, or scene settings change. These CPU changes require an on-device frame-time comparison after deployment; they do not reduce eye resolution or shadow quality.
...
- Standalone headset materials cap texture anisotropy at 8x, including textures that previously held a 16x value. This keeps the eye framebuffer and native MSAA unchanged. The user reported no visible blur or shimmer in the current Quest scene, but a fixed-view FPS benefit has not been established; validate other scenes before treating 8x as a universal visual acceptance.
- The retained `vrHeadsetStereoPostFxEnabled` scene-contract field enables the full PMNDRS/Takram integration; it no longer means that immersive frames necessarily execute a stereo composer. `postProcessing.nativeHeadset` diagnostics and owner `native-headset` identify the actual native presentation path. Force-stereo debug flags retain a reference path for developer A/B checks, not an author-facing build option.
- Takram clouds, scene probes, Takram sky PMREM capture, native WebXR layers, old movement HUDs, and old movement emitter scaffolding remain disabled by default. Do not substitute static headset palettes, A-Frame fallback environments, or fixed lower-hemisphere fills for dynamic Takram sky/time/light state.
...
- Full headset builds preserve the quality atmosphere profile and do not select the older palette/calibrated low-cost direct-sky path. The native output hook is active only during native scene draws and is disabled for composer/probe draws.
