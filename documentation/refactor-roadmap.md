...
### Reflection component lifecycle ownership (2026-09-13)

`vrodos-reflections` now owns HDR request identity/status, HDR and probe render targets, the shared cube camera/PMREM generator, Takram sky PMREM, capture timing/vectors, and reflection-specific interpolation state. The existing scene-settings helper signatures delegate to the component; diagnostic and composer readers use read-only state views. Reflection policy and material traversal remain in their existing modules. Shared scene queries, quality-refresh scheduling/listeners, and atmosphere resources retain their existing owners. Inactive profiles do not allocate a reflection owner through helper calls.

Scene-settings teardown removes the reflection component after composer shutdown. Removal invalidates pending HDR loads, disposes resources, detaches the camera, clears its installed environment, and releases the settings reference, including when A-Frame has already removed the component-map entry. Late successes dispose their input textures; late failures cannot affect a replacement owner. HDR conversion and shader preparation failures release temporary textures/generators. Repeated removal and reattachment are covered. The now-unused scene-settings disposal wrapper was removed.

AST comparison against HEAD verified that 20 of the 21 existing scene-probe/shared helper bodies are identical after normalizing owner-to-settings routing. The remaining helper contains the intended HDR request/cleanup changes. Capture thresholds, resolution/filtering, exclusion rules, retry timing, Takram single-PMREM policy, intensity formulas, shaders, dependencies, and navigation are unchanged. The old Takram implementation-text assertion was replaced by executed lifecycle coverage.

All 71 regression scripts pass (42 runtime, 29 compiler). Tests exercise the actual component/helpers with real Three resources and controlled HDR/PMREM/capture doubles, including source and resolution changes, independent scenes, stale loads, conversion failures, repeated teardown, capture guards/restoration, static and slow-dynamic timing, Takram readiness retries/single capture, smoothing, and authored scene-settings teardown. Runtime/generated-bundle syntax, build configuration, catalog coverage, and diff checks pass. Full browser-source lint has zero errors and 247 existing warnings; the changed sources introduce no new warnings. `npm run build:runtime` succeeded; only the core and A-Frame components bundles changed among generated outputs.
