...
const spatialUiThreeShimPath = path.join(outputDir, '.vrodos-spatial-ui-three-shim.mjs');
const spatialUiFontDir = path.join(rootDir, 'assets', 'vendor', 'fonts', 'noto-sans');
const spatialUiMsdfVendorDir = path.join(rootDir, 'assets', 'vendor', 'zappar-msdf-generator');
const spatialUiMsdfWorkerEntry = path.join(rootDir, 'node_modules', '@zappar', 'msdf-generator', 'dist', 'worker.js');
const spatialUiMsdfWasmSource = path.join(rootDir, 'node_modules', '@zappar', 'msdf-generator', 'dist', 'msdfgen_wasm.wasm');
const spatialUiMsdfLicenseSource = path.join(rootDir, 'node_modules', '@zappar', 'msdf-generator', 'LICENSE');
...
const chunks = [
  ...
  },
  {
    ...
    bundle: true,
    files: [
      path.join(runtimeDir, 'spatial-ui/vrodos_spatial_ui.js')
    ]
  },
  {
  ...
  },
  {
    ...
    files: [
      ...
      path.join(runtimeMasterDir, 'vrodos_runtime_profile_policy.js'),
...
async function buildSpatialUiMsdfAssets() {
  ...
  console.log(`Built ${path.relative(rootDir, spatialUiMsdfWorkerOutput)}`);
  console.log(`Copied ${path.relative(rootDir, spatialUiMsdfWasmOutput)}`);
}
...
function createAliasPlugin(aliases) {
  return {
    name: 'vrodos-runtime-alias-plugin',
    setup(buildContext) {
...
async function buildBundledChunk(chunk) {
  const outputPath = path.join(outputDir, chunk.output);
  await writeGlobalShim('three', 'window.THREE || (window.AFRAME && window.AFRAME.THREE) || {}', spatialUiThreeShimPath);

  try {
    await build({
      entryPoints: chunk.files,
      bundle: true,
      format: 'iife',
      platform: 'browser',
      target: ['es2019'],
      ...
      plugins: [
        createAliasPlugin({
          three: spatialUiThreeShimPath
        })
      ]
    });
  } finally {
    await rm(spatialUiThreeShimPath, { force: true });
  }
...
async function validateBuildInputs() {
  ...
  await assertReadable(path.join(spatialUiFontDir, 'NotoSans-Regular.ttf'), 'Spatial UI Noto Sans regular font');
  await assertReadable(path.join(spatialUiFontDir, 'NotoSans-Bold.ttf'), 'Spatial UI Noto Sans bold font');
  await assertReadable(path.join(spatialUiFontDir, 'LICENSE'), 'Spatial UI Noto Sans license');
  await assertReadable(spatialUiMsdfWorkerEntry, 'Spatial UI MSDF worker source');
  await assertReadable(spatialUiMsdfWasmSource, 'Spatial UI MSDF WASM source');
  await assertReadable(spatialUiMsdfLicenseSource, 'Spatial UI MSDF license source');
  for (const chunk of chunks) {
    for (const file of chunk.files) {
