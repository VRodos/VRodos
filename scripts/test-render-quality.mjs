...
self.data.postFXEngine = 'legacy';
self.data.vrRuntimeProfile = 'headset';
self.applyRenderQualityProfile();
assert.equal(renderer.toneMapping, THREE.NeutralToneMapping, 'headset direct rendering uses the authored tone mapping');
assert.equal(renderer.toneMappingExposure, 1.4, 'headset direct rendering uses the authored exposure');
self.data.vrRuntimeProfile = 'desktop';
self.data.postFXEngine = 'pmndrs';
...
assert.equal(calls[0].options.headsetProfile, false);
...
self.isVrRuntimeHeadsetProfile = () => true;
self.applyMaterialProfiles();
assert.equal(calls.at(-1).options.headsetProfile, true, 'headset material policy reaches texture filtering');
self.isVrRuntimeHeadsetProfile = () => false;
smoothingMs = 0;
