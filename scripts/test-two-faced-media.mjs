...
control.videoAutoplay = true;
control.videoLoop = false;
control.prepareVideoElement(video);
assert.equal(video.loop, false);
video.pause();
components['video-controls'].checkAutoplay.call(control);
assert.equal(video.paused, false, 'Autoplay starts independently of loop.');
assert.equal(video.muted, true, 'Autoplay starts muted so browsers can allow it.');
now += 400;
display.emit('click', {});
assert.equal(video.paused, false, 'Clicking a muted autoplay video keeps it playing.');
assert.equal(video.muted, false, 'Clicking an autoplay video enables sound.');
video.paused = true;
video.emit('ended');
for (const hint of hints) assert.equal(hint.getAttribute('visible'), 'true');
