...
| Reflections | In Progress | `Master_Client_766.html?vrodos_vr_profile=hdr-reflections&vrodos_debug_runtime_features=1` | Loader may remain visible while `reveal.pending` contains `takram-sky` or `hdr-env-map`; after reveal, `reveal.ready=true`, `takram.visibleSkyDirectPatchFailed=false`, `takram.visibleSkyDirectWarmed=true`, `reflections.effectiveSource=hdr`, `reflections.hdrEnvMapOnly=true`, `reflections.effectiveEnvMapPreset=studio`, `reflections.hdrReady=true`, `reflections.hdrFailed=false`; scene probe/Takram sky PMREM/composer/clouds remain disabled | Quest Browser diagnostics pass after loader-gated Takram reveal; user visual and immersive headset acceptance still pending | Recompile `Master_Client_766.html`, retest refresh/startup stability and controller alignment, then inspect HDR reflections visually and enter VR; accept only if reflections are visible and no browser/compositor instability appears |
...
| `Master_Client_766.html` | Intentional HDR/env-map reflection fixture for the next stage | Compiled as Desktop with PMNDRS/Takram chunks, `reflectionsEnabled=1`, `reflectionSource=hdr`, and material/mirror assets. Use Quest URL `http://localhost:5832/wp-content/plugins/VRodos/runtime/build/Master_Client_766.html`, not `wp.local`. Test with hidden profile `vrodos_vr_profile=hdr-reflections`; it enables only HDR/env-map reflections and defaults `envMapPreset=studio` when the scene authored preset is `none`. |
...
- 2026-06-16 headset issue: in `Master_Client_766.html`, user saw controller rays/lines below the active view and reported the immersive view was offset in X/Z as well as height. Treat this as a `vrexpo_games` XR camera-rig split, not a global navigation clamp issue. The compiler/runtime now split the authored camera position into a ground/reference `#player` position plus `cameraA` local eye offset (`0 1.6 0`), so Desktop/tab mode keeps the same authored camera world position while immersive XR can use the headset pose relative to the correct scene anchor. Diagnostics now expose `cameraRig.activeCameraId`, `cameraRig.activeCameraMatchesExpected`, `cameraRig.expectedPlayerPosition`, `cameraRig.expectedCameraLocalPosition`, and world positions for player/camera/controllers.
