...
| --- | --- | --- |
| `Master_Client_8606.html` | Current built test page and accepted VR Headset baseline | Test through `http://localhost:5832/wp-content/plugins/VRodos/runtime/build/Master_Client_8606.html` on Quest via ADB reverse, or through the matching IP URL when needed. If the page was compiled as `VR Headset`, it intentionally omits PMNDRS/Takram chunks and can only validate that `takram-lights` fails closed. If the same scene was rebuilt through the public `Desktop` compile target with Takram atmosphere enabled, use the hidden `vrodos_vr_profile=takram-lights` query to validate Takram light sources while keeping VR-only composer/cloud/reflection paths disabled. |
