...
Takram CloudsEffect is created before AerialPerspectiveEffect when cloud settings are active. When AerialPerspectiveEffect is present, VRodos keeps `cloudsEffect.skipRendering = true`, matching Takram's documented R3F route: CloudsEffect renders its internal cloud buffers, skips direct color-buffer composition, and routes `cloudsEffect.atmosphereOverlay` into AerialPerspectiveEffect for the final cloud overlay. Horizon does not route Takram cloud `atmosphereShadow` / `atmosphereShadowLength` or direct `SkyMaterial.shadowLength` by default because those screen-space shadow buffers can reveal large geometric footprints on the already-rendered Horizon sky; use `?vrodos_debug_enable_pmndrs_cloud_aerial_shadow=1` and `?vrodos_debug_enable_pmndrs_cloud_sky_shadow_length=1` only for comparison. Clouds are therefore a PMNDRS/Takram atmospheric composition feature, not separate scene geometry.
...
- Horizon `AerialPerspectiveEffect` is constrained to cloud overlay/haze/transmittance in the current PBR path so it does not render its own sky or re-light the scene as albedo. The Takram `SkyMaterial` remains the visual sky owner. Its native sun disk is used by default and remains behind high/ultra accurate cloud phase composition; only the fallback desktop cloud sun-disk sprite bridge disables it when the projected disk is cloud-covered.
- Optional Takram clouds synchronize every frame with the active Takram atmosphere, camera, sun direction, world-to-ECEF matrix, correct-altitude mode, precomputed atmosphere textures, local cloud textures, quality profile, authored coverage, effective coverage diagnostics, and Horizon cloud haze ownership.
...
- VRodos Horizon keeps Takram's default `coverageFilterWidth`, default `weatherExponent`, and default `localWeatherRepeat`. The default style applies Takram's documented default layer set. Non-default style presets adjust only conservative layer fields such as altitude, height, density, and shape amount. The coverage mapper fixes the authoring-contract mismatch without changing Takram's weather sharpness/repetition controls. Diagnostics report both authored coverage and effective Takram shader coverage.
...
- Cloud shadow-length output stays routed through `AerialPerspectiveEffect` only outside Horizon, or when `?vrodos_debug_enable_pmndrs_cloud_aerial_shadow=1` is set. The paired AerialPerspective cloud `shadow` buffer is cleared whenever `shadowLength` is cleared, so below-horizon/day-night frames and Horizon default frames cannot carry a stale screen-space cloud-shadow buffer. Horizon also disables direct `SkyMaterial.shadowLength` routing by default because Takram's screen-space shadow-length texture can reveal a delayed rectangular footprint on the already-rendered Horizon sky during day/night motion. Use `?vrodos_debug_enable_pmndrs_cloud_sky_shadow_length=1` only for comparison. Do not draw the sun or lens flare above clouds as a separate ordering hack.
- Because VRodos uses Takram's documented light-source path for PBR materials, active daytime clouds also drive a CPU-only sun-occlusion bridge on `SunDirectionalLight`, `SkyLightProbe`, hemisphere fill, ambient bounce, reflection intensity, direct sun shadow opacity/softness, Takram lens flare, and the visible desktop sun disk. The bridge combines authored cloud coverage with a desktop Horizon sample of Takram cloud opacity around the projected visible sun disk, so a cloud crossing the sun can dim the next rendered frames without enabling incompatible `AerialPerspectiveEffect.sunLight` / `skyLight` post-process lighting or adding a projected cloud-shadow pass. High and ultra profiles mirror Takram's Basic demo more closely by enabling `accuratePhaseFunction`, `accurateSunSkyLight`, `multiScatteringOctaves=8`, `shadow.farScale=1`, and `shadow.maxFar=100000`; in that mode the native Takram disk stays behind light/moderate cloud composition and diagnostics report `cloudSkySunDiskMode=takram-phase`. For very dense sampled disk cover, VRodos hides only the native `SkyMaterial.sun` disk while leaving Takram cloud phase scattering active; diagnostics report `cloudSkySunDiskMode=takram-phase-muted` and `cloudSkySunDiskNativeHidden=true`. The older public-`sun`-flag plus VRodos sprite bridge remains a fallback for lower/debug paths. It does not patch Takram's sky shader. It keeps mesh shadows casting, reduces their contrast by dimming direct sun and moderately lowering `shadow.intensity`, and increases shadow radius under heavy occlusion without making shadows weightless. It is horizon-gated, so it intentionally reports `sun-below-horizon` and leaves moon/night readability unchanged when the sun direct light is off.
...
- High and ultra cloud profiles can enable Takram `CloudsEffect.lightShafts`, but Horizon disables them by default because high-coverage local scenes exposed large screen-space shaft/shadow footprints. Use `?vrodos_debug_enable_pmndrs_cloud_light_shafts=1` only for comparison. Low and medium keep light shafts disabled for cheaper preview profiles, and night/below-horizon scenes clear cloud `shadowLength` routing so the sky is not darkened by a flat sun-shadow buffer.
...
Light shafts are reserved for high/ultra desktop profiles because they are a documented cloud visual feature with extra GPU cost, but Horizon keeps them debug-only until visual/performance validation is complete. The `Haze` column means Takram cloud haze is still a profile feature, but Horizon mode disables it at runtime because SkyMaterial/AerialPerspective already own sky haze there. Accurate phase is enabled only on high/ultra because it is the cloud/sun visual quality path we want authors to use when evaluating final desktop output.
