...
- High/ultra desktop profiles generate Takram cloud shadow-length independently of Aerial consumer readiness and authored coverage, matching the upstream demo lifecycle. The resulting output routes through `AerialPerspectiveEffect` when the masked Aerial path is ready, the sun is above the horizon, and the renderer is non-XR desktop. The shared cloud-owned `NormalPass` must survive later SSAO setup and be reused by SSAO when enabled. The paired AerialPerspective cloud `shadow` buffer is cleared whenever `shadowLength` is cleared, so below-horizon/day-night frames cannot carry a stale screen-space cloud-shadow buffer. Horizon keeps direct `SkyMaterial.shadowLength` routing disabled because Takram's screen-space shadow-length texture can reveal a delayed rectangular footprint on the already-rendered Horizon sky during day/night motion. Do not draw the sun or lens flare above clouds as a separate ordering hack.
...
- Projected moving cloud shadows on terrain/meshes as a separate GPU feature, not as part of the current global cloud-sun bridge.
