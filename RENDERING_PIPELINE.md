...
- `pmndrsCloudsCoverage`: authored cloudiness `0..1`. Horizon maps dense authored values to a lower Takram `CloudsEffect.coverage` shader input because Takram's default layer coverage math saturates near `1 - coverageFilterWidth`.
...
- Authored coverage remains compatible as `0..1`, but Horizon maps it before assigning `CloudsEffect.coverage`: authored values up to `0.35` pass through, and `0.35..1.0` smoothly compresses into roughly `0.35..0.395`. Takram's shader computes density with `mix(localWeather, 1.0, coverageFilterWidth)` and default `coverageFilterWidth ~= 0.6`, so shader coverage much above `0.4` gives density to zero-weather texels and can turn a local Horizon sky into a broad slab.
- VRodos Horizon keeps the default style close to Takram's documented layer set. Dense `overcast` and `storm` styles use higher local-weather repeat, deterministic offset, lower near-layer weather blending, and reduced near-layer density to reduce Takram cube-sphere weather seam/block artifacts. Very dense storm coverage softens the lowest scud layer so nearby clouds remain broken rather than becoming one flat slab. Diagnostics report authored/effective coverage, weather repeat/offset, seam mitigation, requested layer signature, and applied layer signature.
