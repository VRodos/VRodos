...
- Cloud shadow-length output routes through `AerialPerspectiveEffect` for high/ultra desktop shafts only when the masked Aerial path is ready, the sun is above the horizon, coverage is high enough, and the renderer is non-XR desktop. The paired AerialPerspective cloud `shadow` buffer is cleared whenever `shadowLength` is cleared, so below-horizon/day-night frames cannot carry a stale screen-space cloud-shadow buffer. Horizon keeps direct `SkyMaterial.shadowLength` routing disabled because Takram's screen-space shadow-length texture can reveal a delayed rectangular footprint on the already-rendered Horizon sky during day/night motion. Do not draw the sun or lens flare above clouds as a separate ordering hack.
...
- High and ultra cloud profiles enable Takram `CloudsEffect.lightShafts` through the masked Aerial sky path when clouds are active, masked Aerial resources are ready, the sun is above the horizon, effective coverage is high enough, and the renderer is non-XR desktop. Low and medium keep light shafts disabled for cheaper preview profiles, and night/below-horizon scenes clear cloud `shadowLength` routing so the sky is not darkened by a flat sun-shadow buffer. `?vrodos_debug_disable_pmndrs_cloud_light_shafts=1` disables the path for comparison.
