...
- VR feature parity is selected in the compile UI as `Runtime Target`: `Desktop` or `VR Headset`. Internally this is still stored as `scene-settings.vrRuntimeProfile`: `Desktop` maps to `desktop`, and `VR Headset` maps to the accepted `baseline` headset profile. Desktop remains the reference pipeline and `desktop` disables headset-specific overrides. `baseline` is the headset starting point and applies immediately in inline/browser-tab mode as well as immersive VR; it keeps the A-Frame horizon/environment while disabling PMNDRS composer, Takram visible sky/clouds, scene probes, and experimental reflections. Baseline keeps native renderer antialiasing because PMNDRS AA is intentionally disabled in that profile. Baseline compiles without PMNDRS/Takram runtime chunks even if authored Desktop settings still use them. `safe` is the hidden scene-owned parity validation profile: it keeps the same direct-stereo/A-Frame-horizon safety policy as `baseline` while allowing shadows, tone mapping/exposure, material profiles, and diagnostics to be checked. `takram-lights` is a hidden lighting-only validation profile: it keeps the visible A-Frame horizon, blocks PMNDRS/legacy composer ownership, clouds, scene probes, and reflections, and attaches real Takram light sources only when a page was intentionally compiled with PMNDRS/Takram atmosphere resources. `takram-sky` is the next hidden validation profile: it allows the Takram visible sky/horizon and Takram light sources, but still blocks PMNDRS composer/effects, clouds, scene probes, reflections/Takram sky PMREM capture, and WebXR layers. `balanced` and `max` remain internal later headset stages; `max` remains lab-only until lower stages pass real headset testing. The headset profile also owns a WebXR render budget: `baseline`/`safe`/`takram-lights`/`takram-sky` use framebuffer scale `1.0` with foveation `0.5`, `balanced` uses `0.9` with foveation `0.75`, and `max` uses `1.0` with foveation `0.5`.
- The hidden `takram-sky` profile renders Takram's direct `SkyMaterial` without PMNDRS composer ownership. On Quest it uses a cheaper half-float/basic Takram LUT profile and a direct-sky exposure/gamma calibration hook; this is intentionally scoped to `vrodos_vr_profile=takram-sky` and must not change Desktop or accepted `baseline`/`safe`/`takram-lights` behavior.
- Individual experiment gates can also be enabled with `aframeVrPmndrsComposerEnabled`, `aframeVrSceneProbeEnabled`, `aframeVrTakramSkyEnvironmentEnabled`, `aframeVrCloudsEnabled`, or query flags such as `vrodos_vr_profile=takram-lights`, `vrodos_vr_profile=takram-sky`, `vrodos_vr_profile=max`, and `vrodos_enable_xr_pmndrs_composer=1`. The render budget can be overridden per scene with `aframeVrFramebufferScale` / `aframeVrFoveationStrength`, or per test with `vrodos_vr_framebuffer_scale=...` / `vrodos_vr_foveation=...`.
...
- The active profile and policy are exposed through `window.VRODOS_RUNTIME_FEATURE_STATE.vrProfile`, and applied WebXR budget details are exposed through `window.VRODOS_RUNTIME_FEATURE_STATE.renderer.vrRenderBudget`, alongside post-FX owner, reflection source, Takram lighting/cloud skip state, shadow diagnostics, navigation/collision state, and spatial UI state. The lighting-only stage reports `takram.lightsOnlyRequested`, `takram.lightsOnlyActive`, `takram.lightOwner`, and `takram.lightsOnlyUnavailableReason`; the visible-sky stage also reports `takram.visibleSkyActive`, `takram.visibleSkyDirectCalibrated`, `takram.visibleSkyDirectExposure`, and `takram.atmosphereProfile`.
...
| VRodos setting | Takram preset | Resolution scale | Temporal upscale | Shape detail | Turbulence | Light shafts | Notes |
| --- | --- | ---: | --- | --- | --- | --- | --- |
| `low` | `low` | `0.62` | on | off | off | off | Conservative desktop baseline; visibly grainier by design |
| `medium` | `medium` | `0.82` | on | on | off | off | Balanced preview profile |
| `high` | `high` | `1.0` | on | on | on | off | Full-resolution clarity profile |
| `ultra` | `ultra` | `1.0` | on | on | on | off | Reference profile using Takram ultra shadow/raymarch settings |

All four profiles keep `shadow.farScale = 0.25`, matching Takram's London Storybook cloud demo. Light shafts remain disabled in v1 until they have a separate performance and visual validation pass.
