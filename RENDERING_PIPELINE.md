...
- `pmndrsCloudsCoverage`: authored cloudiness `0..1`. Horizon maps it through a simple style-specific scale and cap before assigning Takram `CloudsEffect.coverage`, keeping coverage monotonic while avoiding local-sky saturation.
...
- Optional Takram clouds synchronize every frame with the active Takram atmosphere, camera, sun direction, world-to-ECEF matrix, correct-altitude mode, precomputed atmosphere textures, local cloud textures, quality profile, authored coverage, effective coverage diagnostics, Horizon cloud haze ownership, masked Aerial lighting state, and weather seam mitigation state.
...
| Coverage | Takram `coverage=0.3` default, demo examples around `0.4` | Authored `0..1` maps through style-specific `scale + bias + max`; it remains monotonic and visibly affects every style. |
| Weather/shape | `localWeatherRepeat=100`, `localWeatherOffset=0`, `shapeRepeat=0.0003`, `shapeDetailRepeat=0.006`, `turbulenceRepeat=20`, `turbulenceDisplacement=350` | Style presets do not override these fields; wind only changes `localWeatherVelocity`. |
...
- Authored coverage remains compatible as `0..1`, and diagnostics report authored/effective coverage plus mapper signature. The mapper is intentionally simple so `0.27`, `0.5`, and high coverage remain visibly distinct for `scattered`, `broken`, `overcast`, and `storm`.
