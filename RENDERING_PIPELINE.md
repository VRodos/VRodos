...
Tone mapping, location, local date/time, cloud style, coverage, quality, and wind are the user-facing controls for the atmosphere/cloud setup. Cloud internals such as phase function, multi-scattering octaves, cloud shadow far distance, weather repeat, haze, scattering coefficients, raymarch iteration counts, and cloud shadow routing are profile-owned runtime choices with diagnostics, not author-facing sliders in this phase.
...
- High and ultra cloud profiles enable Takram `CloudsEffect.lightShafts` whenever clouds are active, the sun is above the horizon, and the renderer is non-XR desktop. Buffer generation does not wait for Aerial/mask readiness and does not use a coverage threshold; sparse or zero coverage naturally produces little or no shaft signal in Takram's shader. Routing still waits for the masked Aerial sky path. Low and medium keep light shafts disabled for cheaper preview profiles, and night/below-horizon scenes clear cloud `shadowLength` routing so the sky is not darkened by a flat sun-shadow buffer. `?vrodos_debug_disable_pmndrs_cloud_light_shafts=1` disables the path for comparison.
