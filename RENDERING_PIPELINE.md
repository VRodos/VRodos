...
Tone mapping, location, and local date/time remain the user-facing controls for the atmosphere/celestial setup. Cloud internals such as phase function, multi-scattering octaves, cloud shadow far distance, weather repeat, haze, and cloud shadow routing are profile-owned runtime choices with diagnostics, not author-facing sliders in this phase.
...
- VRodos Horizon keeps Takram's default cloud layers, default `coverageFilterWidth`, default `weatherExponent`, and default `localWeatherRepeat`. The coverage mapper fixes the authoring-contract mismatch without changing Takram's weather sharpness/repetition controls. Diagnostics report both authored coverage and effective Takram shader coverage.
