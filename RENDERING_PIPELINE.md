...
- `pmndrsCloudsLightShaftsEnabled`: author request for the existing cloud-aware Takram light-shafts path; defaults on and does not change cloud quality
...
- High and ultra cloud profiles enable Takram `CloudsEffect.lightShafts` only when `pmndrsCloudsLightShaftsEnabled` is authored on, clouds are active, the sun is above the horizon, and the renderer is non-XR desktop. Buffer generation does not wait for Aerial/mask readiness and does not use a coverage threshold; routing still waits for the masked Aerial sky path. Unchecking the build-dialog control skips masked-Aerial resource preparation and reports `lightShaftsRequested=false` / `lightShaftsSkippedReason=author-disabled` without disabling clouds, cloud overlay, or the cloud-sun lighting bridge. Low and medium report `profile-disabled`; night/below-horizon scenes report `sun-below-horizon` and clear cloud `shadowLength` routing. `?vrodos_debug_disable_pmndrs_cloud_light_shafts=1` remains available for comparison.
...
- PMNDRS cloud diagnostics include `cloudsActive`, `cloudsSkippedReason`, `quality`, `profile`, `takramQuality`, `style`, `layerProfile`, `cloudProfileValidationStatus`, `cloudProfileFallbackReason`, `cloudWindEnabled`, `cloudWindSpeed`, `cloudWindDirectionDeg`, `cloudWindVelocityX`, `cloudWindVelocityY`, `resolutionScale`, `multiScatteringOctaves`, `accurateSunSkyLight`, `accuratePhaseFunction`, `accuratePhaseFunctionSkippedReason`, `shadowFarScale`, `shadowMaxFar`, `temporalUpscale`, `temporalUpscaleSkippedReason`, `haze`, `hazeDisabledReason`, `directCompositeEnabled`, `aerialOverlayRouted`, `lightShaftsRequested`, `lightShafts`, `lightShaftsSkippedReason`, `cloudLightShaftsMode`, `cloudLightShaftsResourcesPrepared`, `cloudShadowLengthGenerationEnabled`, `cloudShadowLengthBufferReady`, `cloudAtmosphereShadowLengthReady`, `cloudLightShaftsVisible`, `aerialSkyEnabled`, `aerialSunLight`, `aerialSkyLight`, `aerialNormalBufferReady`, `aerialLightingMaskReady`, `aerialShadowLengthBufferReady`, `aerialShadowLengthDefineReady`, `aerialLightingMaskDefineReady`, `aerialLightingMaskLayer`, `aerialLightingMaskSelectedCount`, `cloudWeatherSeamMitigation`, `cloudWeatherUvMode`, `cloudWeatherUvPatchApplied`, `cloudWorldToEcefFrame`, `cloudCoverageMapperShared`, `cloudWeatherRepeatX/Y`, `cloudWeatherOffsetX/Y`, `coverage`, `authoredCoverage`, `effectiveCoverage`, `aerialShadowRouted`, `aerialShadowReason`, `skyShadowLengthRouted`, `skyShadowLengthReason`, `lensFlareCloudFactor`, `cloudSunOcclusionEnabled`, `cloudSunOcclusionStrength`, `cloudSunOcclusionReason`, `cloudSunCoverageStrength`, `cloudSunDiskOcclusion`, `cloudSunDiskStrength`, `cloudSunDiskSampleReason`, `cloudSunDiskSampleAgeMs`, `cloudSunDiskSampleCount`, `cloudSunOcclusionTargetStrength`, `cloudSunDirectFactor`, `cloudSkyFactor`, `cloudFillFactor`, `cloudAmbientFactor`, `cloudReflectionFactor`, `cloudSunShadowIntensityFactor`, `cloudSunShadowRadiusScale`, `cloudSkySunDiskVisibility`, `cloudSkySunDiskTargetVisibility`, `cloudSkySunDiskSpriteOpacity`, `cloudSkySunDiskScreenOpacity`, `cloudSkySunDiskNativeHidden`, `cloudSkySunDiskMode`, `cloudSunElevationFactor`, texture readiness/counts, and XR skip state.
