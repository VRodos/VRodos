...
- Visible desktop sun-disk attenuation now prefers Takram's accurate cloud phase path for high and ultra cloud quality. Those profiles apply `accuratePhaseFunction=true`, `accurateSunSkyLight=true`, `multiScatteringOctaves=8`, `shadow.farScale=1`, and `shadow.maxFar=100000`, then keep the native Takram sky sun behind the cloud/AerialPerspective composition (`cloudSkySunDiskMode=takram-phase`). When the sampled projected disk is very densely covered, VRodos disables only the native `SkyMaterial.sun` disk and leaves Takram cloud phase scattering active (`cloudSkySunDiskMode=takram-phase-muted`) so solid clouds show a veiled cloud glow instead of a hard white disk. This matches the Takram Basic demo more closely than a billboard disk.
