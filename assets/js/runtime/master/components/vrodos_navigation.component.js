...
AFRAME.registerComponent('custom-movement', {
    ...
    calculateVerticalMotionStep: function (deltaSeconds, output) {
        ...
        if (gravity > 0 && unconstrainedVelocity < terminalVelocity) {
            ...
            output.deltaY = (startVelocity * timeToTerminal) -
                (0.5 * gravity * timeToTerminal * timeToTerminal) +
                (terminalVelocity * (dt - timeToTerminal));
    ...
    requestJump: function (source) {
        ...
        if (
            !settings ||
            movementDisabled ||
            this.getNavigationMode(settings) !== 'walkable' ||
            !this.areCollisionsEnabled(settings) ||
            this.isAirborne() ||
            !this.hasLastGroundHit ||
            this.heightOffset === null
        ) {
            return false;
        }
        ...
        if (
            !supportGround ||
            Math.abs(supportGround.point.y - footY) >
                (this.verticalCollisionSkin + this.groundProbeStepTolerance)
        ) {
    ...
    hasActiveNavigationInput: function () {
        const deadzone = this.data && Number.isFinite(this.data.thumbstickDeadzone)
            ? this.data.thumbstickDeadzone
            : 0.08;
        ...
        return Math.abs(input.x || 0) > 0.0001 ||
            Math.abs(input.y || 0) > 0.0001 ||
            Math.abs(input.vertical || 0) > 0.0001 ||
            Math.abs(keyboard.x || 0) > 0.0001 ||
            Math.abs(keyboard.y || 0) > 0.0001 ||
            Math.abs(keyboard.vertical || 0) > 0.0001 ||
            Math.abs(left.x || 0) > deadzone ||
            Math.abs(left.y || 0) > deadzone ||
            Math.abs(right.x || 0) > deadzone ||
            Math.abs(right.y || 0) > deadzone;
    },
    ...
    restoreDesktopFullscreenNavigationPose: function (reason, attemptIndex) {
        ...
        if (diagnostics.distance < 0.0001) {
    ...
    observeImmersiveViewerPose: function () {
        ...
        this.lastImmersiveAnchorCorrection = {
            ...
            referenceSpaceType: this.sceneEl && this.sceneEl.systems && this.sceneEl.systems.webxr
                ? this.sceneEl.systems.webxr.sessionReferenceSpaceType : 'unknown'
        };
    ...
    resetImmersiveHeight: function () {
        ...
        const previousHeight = Number.isFinite(this.heightOffset)
            ? this.heightOffset
            : this.getDesiredImmersiveEyeToGroundOffset();
        const ground = this.areCollisionsEnabled()
            ? this.sampleGroundAt(
                navigationPosition,
                this.hasLastGroundHit ? this.lastGroundHit.point.y : undefined,
                this.sampledGroundHit
        ...
        this.immersiveHeightCalibrationApplied = false;
        this.immersiveHeightSource = 'reset';
        // Reanchor only vertically. Reanchoring X/Z would shift the user's position
        // when they have moved physically within their playspace.
    ...
    findWalkableGroundBelowAt: function (position, maxGroundY, outputGround) {
        ...
        this.refreshNavMeshRoots();
        this.refreshCollisionWorld();
        const targets = this.blockerCollisionTargets.length > 0
            ? this.blockerCollisionTargets
            : this.navMeshCollisionTargets;
        ...
        const maxY = Number.isFinite(maxGroundY)
            ? maxGroundY
            : position.y - (this.heightOffset !== null ? this.heightOffset : 1.6);
        ...
        const immersivePresenting = this.isImmersiveXrPresenting();
        const rayDirection = immersivePresenting
            ? this.authoredToRenderedDirection(this.raycastDirection, this.verticalRenderedRayDirection)
            : this.raycastDirection;
        const verticalSpan = bounds && !bounds.isEmpty()
            ? Math.max(0, bounds.max.y - bounds.min.y)
            : 0;
        const rayDistance = Math.max(
            10,
            verticalSpan +
                (typeof this.heightOffset === 'number' ? this.heightOffset : 1.6) +
                (Number(this.jumpHeight) || VRODOSVerticalMovementDefaults.jumpHeight) +
                2
        );
        ...
        for (let offsetIndex = 0; offsetIndex < offsetCount; offsetIndex++) {
            ...
            const rayOrigin = immersivePresenting
                ? this.authoredToRenderedPosition(this.verticalRayOrigin, this.verticalRenderedRayOrigin)
                : this.verticalRayOrigin;
            ...
            for (let hitIndex = 0; hitIndex < intersections.length; hitIndex++) {
                ...
                const authoredHitPoint = immersivePresenting
                    ? this.renderedToAuthoredPosition(hit.point, this.verticalAuthoredHitPoint)
                    : hit.point;
                ...
                if (
                    this.getCollisionRoleFromObject(hit.object) !== 'navmesh' ||
                    slope > this.data.maxSlope + 0.5
                ) {
    ...
    getUpwardVerticalClearance: function (position, footY, requestedDistance) {
        ...
        for (let i = 0; i < this.verticalCapsuleOffsets.length; i++) {
            ...
            const rayOrigin = immersivePresenting
                ...
                : this.verticalRayOrigin;
            const rayDirection = immersivePresenting
                ? this.authoredToRenderedDirection(this.verticalRayDirection, this.verticalRenderedRayDirection)
                : this.verticalRayDirection;
    ...
    resolveMovementAgainstBlockers: function (currentPosition, deltaX, deltaZ, currentGround, resolvedStep) {
        ...
        const blockerOptions = resolvedStep.airborne
            ? null
            : this.getAutoStepNavmeshBlockerOptions(currentGround, resolvedStep.ground);
        ...
        const trySlideStep = (candidateDeltaX, candidateDeltaZ, outputStep) => {
            ...
            const slideBlockerOptions = candidateStep.airborne
                ? null
                : this.getAutoStepNavmeshBlockerOptions(currentGround, candidateStep.ground);
    ...
    applyAirborneMovement: function (deltaX, deltaZ) {
        ...
        if (this.resolveAirborneHorizontalCandidate(currentPosition, this.airborneTargetPosition, footY)) {
            ...
            if (canSlideX && canSlideZ) {
                resolvedPosition = Math.abs(deltaX) >= Math.abs(deltaZ)
                    ? this.airborneSlidePositionX
                    : this.airborneSlidePositionZ;
            } else if (canSlideX) {
    ...
    updateVerticalMotion: function (timeDelta, settings, collisionsEnabled) {
        ...
        const motion = this.calculateVerticalMotionStep((Math.min(Math.max(Number(timeDelta) || 0, 0), 50)) / 1000);
        ...
        if (deltaY > 0 || motion.velocity > 0) {
            ...
            if (
                landingGround &&
                landingGround.point.y <= footY + this.verticalCollisionSkin &&
                nextFootY <= landingGround.point.y + this.verticalCollisionSkin
            ) {
            ...
            if (
                !landingGround &&
                this.lastVerticalSupportStatus === 'blocked-solid' &&
                Number.isFinite(this.lastVerticalSupportBlockedGroundY) &&
                nextFootY <= this.lastVerticalSupportBlockedGroundY + this.verticalCollisionSkin
            ) {
    ...
    syncHeightOffset: function () {
        ...
        const navigationPosition = this.getNavigationWorldPosition();
        const currentGround = this.sampleGroundAt(navigationPosition, this.hasLastGroundHit ? this.lastGroundHit.point.y : undefined, this.sampledGroundHit);
        if (!currentGround) {
            return;
        }
