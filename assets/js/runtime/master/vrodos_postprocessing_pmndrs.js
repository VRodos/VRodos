...
(function () {
    ...
    function getPmndrsCloudSunVisibilityForSelf(self, atmosphereConfig) {
        const config = atmosphereConfig || (self && typeof self.getPmndrsAtmosphereConfig === 'function'
    ...
    function isPmndrsCloudLightShaftsRequested(self) {
        return readPmndrsBool(self, 'pmndrsCloudsLightShaftsEnabled');
    }
    ...
    function shouldPreparePmndrsMaskedAerialSkyLightShafts(self, profile) {
        ...
        if (!isPmndrsCloudLightShaftsRequested(self)) {
            return false;
        }
    ...
    function getPmndrsCloudLightShaftBufferDiagnostics(self) {
        ...
        const atmosphereShadowLength = cloudsEffect && cloudsEffect.atmosphereShadowLength
            ? cloudsEffect.atmosphereShadowLength
            : null;
        const cloudsPassShadowLength = cloudsEffect && cloudsEffect.cloudsPass
            ? cloudsEffect.cloudsPass.shadowLengthBuffer
            : null;
        const aerialShadowLength = aerialEffect && aerialEffect.shadowLength
            ? aerialEffect.shadowLength
            : null;
    ...
    function shouldUsePmndrsCloudHaze(self, profile) {
        ...
        // Takram CloudsEffect includes its own screen-space cloud haze. Horizon
        // already owns sky/haze through SkyMaterial + AerialPerspectiveEffect,
        // so leaving the cloud haze enabled double-composites into a slab.
    ...
    const PMNDRS_CLOUD_PERFORMANCE_PROFILES = {
        ...
        medium: {
            ...
            shadowMaxFar: null,
            localWeatherRepeat: 100,
            localWeatherVelocity: [0.00025, 0],
            shapeRepeat: 0.0003,
            shapeDetailRepeat: 0.006,
            turbulenceRepeat: 20,
            turbulenceDisplacement: 350
        ...
        ultra: {
            ...
            shadowMaxFar: 100000,
            localWeatherRepeat: 100,
            localWeatherVelocity: [0.00035, 0],
            shapeRepeat: 0.0003,
            shapeDetailRepeat: 0.006,
            turbulenceRepeat: 20,
            turbulenceDisplacement: 350
    ...
    function shouldUsePmndrsCloudAccuratePhaseFunction(profile) {
        return Boolean(profile && profile.accuratePhaseFunction === true) &&
            !hasPmndrsDebugFlag('disablePmndrsCloudAccuratePhaseFunction', 'vrodos_debug_disable_pmndrs_cloud_accurate_phase_function');
    }
    ...
    function getPmndrsCloudAccuratePhaseFunctionSkippedReason(profile, enabled) {
        ...
        return hasPmndrsDebugFlag('disablePmndrsCloudAccuratePhaseFunction', 'vrodos_debug_disable_pmndrs_cloud_accurate_phase_function')
            ? 'debug-disabled'
            : 'profile-disabled';
    }

    function getPmndrsCloudDiagnosticNumber(diagnostics, key, fallback) {
        return diagnostics && typeof diagnostics[key] === 'number'
            ? diagnostics[key]
            : fallback;
    }
    ...
    function shouldUsePmndrsCloudLocalWeatherUv(self, atmosphereConfig) {
        ...
        return Boolean(
            ...
            !(atmosphereConfig && atmosphereConfig.geospatialEnabled === true) &&
    ...
    function setPmndrsCloudMaterialDefine(material, defineName, enabled) {
        ...
        if (!next && defines[defineName] !== undefined) {
    ...
    function hasPmndrsCloudMaterialDefine(material, defineName) {
        ...
        return defines instanceof Map ? defines.has(defineName) : defines[defineName] !== undefined;
    }

    function hasPmndrsCloudDefine(effect, defineName, requireShadowMaterial) {
        const cloudMaterial = effect && effect.cloudsPass && effect.cloudsPass.currentMaterial
            ? effect.cloudsPass.currentMaterial
            : null;
        const shadowMaterial = effect && effect.shadowPass && effect.shadowPass.currentMaterial
        ...
        if (requireShadowMaterial) {
            return Boolean(
                cloudMaterial &&
                shadowMaterial &&
                hasPmndrsCloudMaterialDefine(cloudMaterial, defineName) &&
                hasPmndrsCloudMaterialDefine(shadowMaterial, defineName)
            );
        }
    ...
    function applyPmndrsCloudPerformanceProfile(effect, quality, style, self) {
        ...
        const lightShaftsRequested = Boolean(profile.lightShafts && (!self || isPmndrsCloudLightShaftsRequested(self)));
        const signature = [
            profile.id,
            profile.style || '',
        ...
        if (typeof effect.lightShafts !== 'undefined') {
            effect.lightShafts = lightShaftsRequested;
        }
    ...
    function updatePmndrsCloudDiagnostics(self, updates) {
        ...
        const atmosphereConfig = typeof self.getPmndrsAtmosphereConfig === 'function'
            ? self.getPmndrsAtmosphereConfig()
            : null;
        const next = Object.assign({
            ...
            lightShaftsRequested: isPmndrsCloudLightShaftsRequested(self),
            lightShafts: false,
            ...
            lensFlareCloudFactor: typeof self._pmndrsLensFlareCloudFactor === 'number'
    ...
    function updatePmndrsCloudLinkedDiagnostics(self, updates) {
        ...
        Object.assign(diagnostics, {
            ...
            layerProfile: profile.layerProfile || '',
            layerSignature: getPmndrsCloudLayerProfileSignature(profile.layers),
            appliedLayerSignature: self.pmndrsCloudsEffect && self.pmndrsCloudsEffect._vrodosCloudLayerAppliedSignature
                ? self.pmndrsCloudsEffect._vrodosCloudLayerAppliedSignature
                : (diagnostics.appliedLayerSignature || ''),
            layerApplyMethod: self.pmndrsCloudsEffect && self.pmndrsCloudsEffect._vrodosCloudLayerApplyMethod
                ? self.pmndrsCloudsEffect._vrodosCloudLayerApplyMethod
                : (diagnostics.layerApplyMethod || ''),
            ...
            cloudCoverageMapperShared: true,
            cloudWeatherSeamMitigation: profile.weatherSeamMitigation || 'none',
            cloudWeatherUvMode: self.pmndrsCloudsEffect && self.pmndrsCloudsEffect._vrodosCloudWeatherUvMode
                ? self.pmndrsCloudsEffect._vrodosCloudWeatherUvMode
                : getPmndrsCloudWeatherUvMode(self, atmosphereConfig),
            ...
            cloudWorldToEcefFrame: self.pmndrsCloudsEffect && self.pmndrsCloudsEffect._vrodosCloudWorldToEcefFrame
                ? self.pmndrsCloudsEffect._vrodosCloudWorldToEcefFrame
                : getPmndrsCloudWorldToEcefFrame(self, atmosphereConfig),
            cloudWeatherRepeatX: Array.isArray(profile.localWeatherRepeat) ? profile.localWeatherRepeat[0] : profile.localWeatherRepeat,
            cloudWeatherRepeatY: Array.isArray(profile.localWeatherRepeat) ? profile.localWeatherRepeat[1] : profile.localWeatherRepeat,
            cloudWeatherOffsetX: Array.isArray(profile.localWeatherOffset) ? profile.localWeatherOffset[0] : 0,
            cloudWeatherOffsetY: Array.isArray(profile.localWeatherOffset) ? profile.localWeatherOffset[1] : 0,
    ...
    function syncPmndrsCloudShadowLengthToSky(self, reason) {
        ...
        if (helpers && typeof helpers.syncPmndrsCloudShadowLengthToSkyMaterial === 'function') {
            return helpers.syncPmndrsCloudShadowLengthToSkyMaterial.call(self, undefined, reason || 'cloud-sync');
        }
...
`;
    ...
    function markPmndrsCloudsSkipped(self, reason, updates) {
        const lightShaftsRequested = isPmndrsCloudLightShaftsRequested(self);
        setPmndrsCloudLightShafts(self && self.pmndrsCloudsEffect, false);
        const next = updatePmndrsCloudDiagnostics(self, Object.assign({
            ...
            lightShafts: false,
            lightShaftsSkippedReason: lightShaftsRequested
    ...
    function loadPmndrsCloudTexture3D(url, size, onLoad, onError) {
        ...
        if (!THREE || typeof THREE.Data3DTexture !== 'function' || !VTC || typeof VTC.DataTextureLoader !== 'function' || typeof VTC.parseUint8Array !== 'function') {
            throw new Error('Takram DataTextureLoader support is unavailable.');
        }

        return new VTC.DataTextureLoader(THREE.Data3DTexture, VTC.parseUint8Array, {
            width: size,
            height: size,
            depth: size,
    ...
    function loadPmndrsCloudStbnTexture(url, onLoad, onError) {
        const THREE = window.THREE;
        const VTC = getPmndrsCloudsBundle();
        if (!THREE || typeof THREE.Data3DTexture !== 'function' || typeof THREE.FileLoader !== 'function') {
    ...
    function ensurePmndrsCloudTextures(self) {
        ...
        if (!VTC || !THREE || !hasCompletePmndrsCloudAssetUrls(urls)) {
            return null;
        }
    ...
    function isPmndrsXrStereoComposerLabEnabled() {
        return hasPmndrsDebugFlag('forceXrPmndrsStereoComposer', 'vrodos_force_xr_pmndrs_stereo_composer') ||
            hasPmndrsDebugFlag('xrPmndrsStereoLab', 'vrodos_xr_pmndrs_stereo_lab');
    }

    function isPmndrsXrStereoSmaaLabEnabled() {
        return hasPmndrsDebugFlag('forceXrPmndrsStereoSmaa', 'vrodos_force_xr_pmndrs_stereo_smaa') ||
            hasPmndrsDebugFlag('xrPmndrsStereoSmaa', 'vrodos_xr_pmndrs_stereo_smaa');
    }

    // Keep authored stereo eligibility for atmosphere/LUT policy. Presentation can
    // use the native target without selecting the older calibrated direct sky.
    function shouldUsePmndrsNativeHeadset(self) {
        return Boolean(self.canUseVrHeadsetStereoPmndrsComposer &&
            self.canUseVrHeadsetStereoPmndrsComposer() &&
            isPmndrsDirectVrPresentationActive(self) &&
            getPmndrsToneMappingMode(self) === 'aces-filmic' &&
            !isPmndrsXrStereoComposerLabEnabled() &&
            !isPmndrsXrStereoSmaaLabEnabled());
    }
    ...
    function prepareNativeHeadsetAtmosphereMaterial(material) {
        ...
        // Capture the old key before replacing onBeforeCompile: Three's default
        // cache key reads that function dynamically.
        const previousKey = cacheKey();
        material.onBeforeCompile = function (shader, renderer) {
            compile.call(this, shader, renderer);
            const uniform = 'void main() {';
            // Takram resolves its MRT chunks before onBeforeCompile. Both raw
            // shaders end with main(); insert after its radiance output.
    ...
    function createNativeHeadsetPresentation(self, renderer) {
        ...
        const diagnostics = self.pmndrsNativeHeadsetDiagnostics = {
            active: false, path: 'composer', antialias: 'composer', skyOutputPatched: false
        };
        ...
        return {
            ...
            render(scene, camera) {
                ...
                renderer.toneMappingExposure = typeof self.getPmndrsToneMappingExposure === 'function'
                    ? self.getPmndrsToneMappingExposure()
                    : readPmndrsNumber(self, 'pmndrsToneMappingExposure', 0.1, 5, 1.0);
                if (output) {
                    output.enabled.value = 1;
                    output.exposure.value = renderer.toneMappingExposure;
                    sky.renderOrder = 10000;
                }
                // Takram's opaque star shader contains sky + star radiance, so it
                // must follow the late sky and receive the same output transform.
                if (starOutput) {
    ...
    function isPmndrsXrStereoComposerRequested(self) {
        return isPmndrsXrStereoComposerLabEnabled() ||
            Boolean(self && typeof self.canUseVrHeadsetStereoPmndrsComposer === 'function' && self.canUseVrHeadsetStereoPmndrsComposer());
    }

    function isPmndrsXrStereoSmaaEnabled(self) {
        return isPmndrsXrStereoSmaaLabEnabled() ||
            Boolean(self && typeof self.canUseVrHeadsetStereoPmndrsComposer === 'function' && self.canUseVrHeadsetStereoPmndrsComposer());
    }
    ...
    function getPmndrsXrStereoCamera(self) {
        ...
        try {
            ...
            return xrCamera && xrCamera.isArrayCamera && xrCamera.cameras && xrCamera.cameras.length > 1
                ? xrCamera
                : null;
        } catch (err) {
            return null;
    ...
    function shouldBypassPmndrsComposerForVr(self) {
        ...
        return isPmndrsDirectVrPresentationActive(self) &&
            !(self && typeof self.canUseVrPmndrsComposer === 'function' && self.canUseVrPmndrsComposer());
    }

    function shouldSkipPmndrsCloudsForVr(self) {
        return isPmndrsDirectVrPresentationActive(self) &&
            !(self && typeof self.canUseVrClouds === 'function' && self.canUseVrClouds());
    }

    function getPmndrsCloudComposerDisabledReason(self) {
        if (self &&
            typeof self.canUsePmndrsComposerOnHeadset === 'function' &&
            !self.canUsePmndrsComposerOnHeadset()) {
    ...
    function isPmndrsNormalPassExcludedObject(node) {
        return Boolean(node && node.userData && (
    ...
    function setPmndrsComposerBufferSizeWithoutRendererResize(self, width, height) {
        ...
        const buffersMatch =
            (!inputBuffer || (inputBuffer.width === w && inputBuffer.height === h)) &&
            (!outputBuffer || (outputBuffer.width === w && outputBuffer.height === h));
    ...
    function isPmndrsCloudsRequested(self) {
        ...
        return Boolean(self &&
            self.data &&
            self.data.postFXEnabled !== '0' &&
            self.data.postFXEngine === 'pmndrs' &&
            self.data.pmndrsAtmosphereEnabled !== '0' &&
            readPmndrsBool(self, 'pmndrsCloudsEnabled'));
    }
    ...
    function getPmndrsCloudSkipReason(self, renderer, atmosphereConfig, textureState) {
        ...
        if (!renderer || !renderer.capabilities || renderer.capabilities.isWebGL2 !== true || !THREE || typeof THREE.Data3DTexture !== 'function') {
    ...
    function shouldEnablePmndrsAerialPerspective(self) {
        return readPmndrsBool(self, 'pmndrsAerialPerspectiveEnabled') ||
            Boolean(self && self._pmndrsCloudsDiagnostics && self._pmndrsCloudsDiagnostics.cloudsActive === true) ||
            (isHorizonBackground(self) &&
    ...
    function refreshPmndrsCloudLightingMaskSelection(self, scene) {
        ...
        if (
            ...
            (
                hasPmndrsDebugFlag('pmndrsHorizonDiagnostics', 'vrodos_debug_pmndrs_horizon') ||
    ...
    function syncPmndrsCloudMaskedAerialSkyLighting(self, atmosphereConfig) {
        ...
        const profile = getPmndrsCloudRuntimeProfile(self, getPmndrsCloudsQuality(self), getPmndrsCloudsStyle(self));
        const allowMaskedAerialSkyLighting = shouldRoutePmndrsCloudLightShafts(self, profile, atmosphereConfig) &&
            isHorizonBackground(self) &&
    ...
    function updatePmndrsAADebugOverlay(self) {
        ...
        lines = [
            ...
            `clouds light shafts: ${  self && self._pmndrsCloudsDiagnostics ? `requested ${self._pmndrsCloudsDiagnostics.lightShaftsRequested ? 'yes' : 'no'}, active ${self._pmndrsCloudsDiagnostics.lightShafts ? `yes ${self._pmndrsCloudsDiagnostics.cloudLightShaftsMode || 'shadow-length-only'}` : `no${self._pmndrsCloudsDiagnostics.lightShaftsSkippedReason ? ` (${self._pmndrsCloudsDiagnostics.lightShaftsSkippedReason})` : ''}`}` : 'off'}`,
    ...
    function disposePmndrsComposerResources(self, pendingEffects = []) {
        ...
        [self.pmndrsRenderPass, self.pmndrsEffectPass, self.pmndrsAtmospherePass,
            self.pmndrsChromaticAberrationPass, self.pmndrsSmaaPass, self.pmndrsLensFlarePass,
            self.pmndrsHorizonFoliageOverlayPass].forEach(disposeUnattached);
        ...
        [self.pmndrsBloomEffect, self.pmndrsLensFlareEffect, self.pmndrsSmaaEffect,
            self.pmndrsLutEffect, self.pmndrsNoiseEffect, self.pmndrsChromaticAberrationEffect,
            self.pmndrsAerialPerspectiveEffect, ...pendingEffects].forEach(disposeUnattached);
    ...
    function routePmndrsCloudsIntoAerial(self, atmosphereConfig) {
        ...
        if (self.pmndrsCloudsEffect) {
            ...
            const aerialShadowLengthReady = Boolean(
                self.pmndrsCloudsEffect.atmosphereShadowLength &&
                self.pmndrsCloudsEffect.atmosphereShadowLength.map
            );
            ...
            updatePmndrsCloudLinkedDiagnostics(self, Object.assign({
                ...
                cloudLightShaftsVisible: Boolean(
                    routeAerialShadowLength &&
                    self.pmndrsAerialPerspectiveEffect.sky &&
                    self.pmndrsAerialPerspectiveEffect.normalBuffer &&
                    self.pmndrsAerialPerspectiveEffect.lightingMask &&
                    shaftBufferDiagnostics.aerialShadowLengthBufferReady &&
                    shaftBufferDiagnostics.aerialShadowLengthDefineReady &&
                    shaftBufferDiagnostics.aerialLightingMaskDefineReady
                )
    ...
    function syncPmndrsCloudsEffect(self, camera, atmosphereConfig) {
        ...
        markPmndrsCloudsActive(self, Object.assign({
            ...
            turbulence: typeof self.pmndrsCloudsEffect.turbulence !== 'undefined'
                ? Boolean(self.pmndrsCloudsEffect.turbulence)
                : Boolean(profile.turbulence),
            haze: hazeEnabled,
            hazeDisabledReason,
            directCompositeEnabled,
            coverage: getPmndrsCloudsCoverage(self),
    ...
    H._buildPmndrsComposer = function (scene, camera) {
        ...
        if (!xrStereoMode && isPmndrsCloudsRequested(this)) {
            ...
            if (cloudsSkipReason) {
                ...
                try {
                    ...
                    const initialLightShaftsEnabled = setPmndrsCloudLightShafts(
                        this.pmndrsCloudsEffect,
                        shouldGeneratePmndrsCloudShadowLength(this, cloudsProfile, atmosphereConfig)
                    );
                    ...
                    updatePmndrsCloudDiagnostics(this, {
                        ...
                        lightShaftsRequested: isPmndrsCloudLightShaftsRequested(this),
                        lightShafts: initialLightShaftsEnabled,
                        lightShaftsSkippedReason: getPmndrsCloudShadowLengthGenerationSkippedReason(
                            this,
                            cloudsProfile,
                            initialLightShaftsEnabled,
                            atmosphereConfig
                        ),
                        ...
                        turbulence: typeof this.pmndrsCloudsEffect.turbulence !== 'undefined'
                            ? Boolean(this.pmndrsCloudsEffect.turbulence)
                            : Boolean(cloudsProfile.turbulence),
        ...
        if (!xrStereoMode && isPmndrsLutEnabled(this) && readPmndrsNumber(this, 'pmndrsLutStrength', 0, 1, 1.0) > 0) {
            try {
    ...
    H.enablePmndrsPostProcessing = function () {
        ...
        renderer.render = function (scene, camera) {
            ...
            const shouldIntercept = self.pmndrsActive &&
                ...
                !shouldBypassPmndrsComposerForVr(self) &&
            ...
            if (
                (self.pmndrsHorizonFoliageOverlayPass || self.pmndrsCloudLightingMaskPass) &&
                shouldRefreshPmndrsSceneSelections(self)
            ) {
                if (self.pmndrsHorizonFoliageOverlayPass) {
