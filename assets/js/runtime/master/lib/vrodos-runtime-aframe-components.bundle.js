...
(() => {
  (function initVrodosModelOrigin(global) {
    ...
    function finiteCenter(center) {
      return Boolean(center) && Number.isFinite(center.x) && Number.isFinite(center.y) && Number.isFinite(center.z);
    }
    ...
    function createOffsetRoot(contentRoot, requestedMode, sourceCenter) {
      ...
      if (contentRoot.userData && contentRoot.userData.vrodosModelOriginWrapper === true && contentRoot.userData.vrodosModelOriginMode === mode) {
        return {
          root: contentRoot,
          applied: true,
          alreadyApplied: true,
  ...
  (function initVrodosGlbLighting(global) {
    const EXTREME_LIGHT_INTENSITY = 1e3;
    ...
    function normalize(root) {
      ...
      const lights = [];
      root.traverse((node) => {
        if ((node.isPointLight || node.isSpotLight || node.isDirectionalLight) && Number.isFinite(node.intensity) && node.intensity > 0 && !(node.userData && node.userData.vrodosGlbLightNormalized)) {
          lights.push(node);
        }
      });
  ...
  AFRAME.registerSystem("vrodos-selection-bvh", {
    ...
    prepareMesh: function(mesh) {
      ...
      if (!mesh.isMesh || mesh.isSkinnedMesh || mesh.isInstancedMesh || !geometry || Object.keys(geometry.morphAttributes || {}).length) return;
      const triangleCount = (geometry.index ? geometry.index.count : ((_a = geometry.attributes.position) == null ? void 0 : _a.count) || 0) / 3;
      if (triangleCount < 2e3 || this.meshes.has(mesh)) return;
  ...
  AFRAME.registerComponent("vrodos-scene-loader", {
    ...
    updateProgress: function() {
      ...
      const downloadState = window.VRODOSSceneLoadProgress && typeof window.VRODOSSceneLoadProgress.snapshot === "function" ? window.VRODOSSceneLoadProgress.snapshot() : null;
  ...
  AFRAME.registerComponent("vrodos-glb-animation", {
    init: function() {
      ...
      this.startAnimation((_a = this.el.components["gltf-model"]) == null ? void 0 : _a.model);
    },
    ...
    tick: function(time, delta) {
      ...
      if (((_a = this.el.components["gltf-model"]) == null ? void 0 : _a.model) !== this.model) {
        this.stopAnimation();
        return;
      }
      if (Number.isFinite(delta) && delta > 0) this.mixer.update(delta / 1e3);
    },
    remove: function() {
      this.resources.disposeAll();
      this.stopAnimation();
  ...
  AFRAME.registerComponent("player-info", {
    ...
    init: function() {
      ...
      this.resources = window.VRODOSMaster.RuntimeResources.createRegistry();
      this.removed = false;
      this.occupancySystem = (_a = this.el.sceneEl) == null ? void 0 : _a.systems["vrodos-chat-occupancy"];
      (_b = this.occupancySystem) == null ? void 0 : _b.updatePlayer(this);
    ...
    update: function() {
      var _a;
      (_a = this.occupancySystem) == null ? void 0 : _a.updatePlayer(this);
      this.applyAvatar();
    },
    remove: function() {
      var _a;
      (_a = this.occupancySystem) == null ? void 0 : _a.unregisterPlayer(this);
  ...
  (function() {
    ...
    AFRAME.registerComponent("vrodos-render-profile", {
      ...
      bindSettings: function(settings) {
        ...
        ["fpsStats", "fpsStatsRoot", "fpsStatsPending", "_vrodosShadowPerfOverlay"].forEach((field) => {
          Object.defineProperty(settings, field, {
            configurable: true,
            get: function() {
              return this.renderProfileRuntime ? this.renderProfileRuntime[field] : null;
            }
          });
        });
      },
      ...
      enableFPSMeter: function() {
        ...
        try {
          ...
          if (typeof this.fpsStats.init === "function" && this.el.renderer) {
            ...
            let initialized;
            try {
              initialized = meter.init(renderer);
            } finally {
      ...
      disableFPSMeter: function() {
        ...
        this.fpsStatsRoot = null;
        try {
          window.VRODOSMaster.RuntimeResources.dispose(meter);
        } catch (error) {
          console.warn("VRodos Error: stats-gl cleanup failed.", error);
        }
      },
    ...
    AFRAME.registerComponent("vrodos-atmosphere", {
      init: function() {
        ...
        this.normalPassExclusions = /* @__PURE__ */ new Map();
        this.removed = false;
    ...
    AFRAME.registerComponent("vrodos-reflections", {
      ...
      tick: function(time) {
        ...
        const staticHeadsetHdr = effectiveReflectionSource === "hdr" && typeof settings.isVrRuntimeHeadsetProfile === "function" && settings.isVrRuntimeHeadsetProfile() && typeof settings.isPmndrsDayNightCycleActive === "function" && !settings.isPmndrsDayNightCycleActive() && !(settings._pmndrsCloudsDiagnostics && settings._pmndrsCloudsDiagnostics.cloudsActive);
        const staticIntensity = this.staticHeadsetHdrIntensity;
        const intensityUnchanged = staticHeadsetHdr && staticIntensity && staticIntensity.data === settings.data && staticIntensity.environment === this.el.object3D.environment && staticIntensity.materials === settings._vrodosReflectionIntensityMaterials;
  ...
  AFRAME.registerComponent("scene-settings", {
    ...
    getRuntimePipelineComponentNames: function() {
      ...
      const authoredEnvMapPreset = (this.data.envMapPreset || "none") !== "none";
      const authoredHdrEnvMap = this.getReflectionSource() === "hdr" || authoredEnvMapPreset;
      const canUseHdrEnvMap = this.areReflectionsEnabled() && authoredHdrEnvMap && this.vrRuntimeAllows("hdrEnvMap", authoredHdrEnvMap);
      const canUseTakramSkyEnvironment = this.areReflectionsEnabled() && this.data.postFXEngine === "pmndrs" && this.isPmndrsAtmosphereEnabled() && this.getVrRuntimeFeaturePolicy().takramSkyEnvironment;
    ...
    getRuntimeRevealReadinessState: function(options) {
      ...
      if (wantsTakramSky) {
        ...
        const usesDirectSkyCalibration = typeof this.usesVrTakramDirectSkyCalibration === "function" ? this.usesVrTakramDirectSkyCalibration() : true;
        if (startLoads && typeof this.prepareVrTakramVisibleSkyForReveal === "function") {
          ...
          takramReady = usesDirectSkyCalibration ? Boolean(shaderPatched && !shaderPatchFailed && skyWarmed) : Boolean(
            atmosphereState2 && atmosphereState2.ready && !atmosphereState2.failed && atmosphereState2.skyMesh && atmosphereState2.skyMaterial
          );
        ...
        state.takramSkyWarmed = usesDirectSkyCalibration ? Boolean(
          atmosphereState && atmosphereState.vrTakramSkyDirectWarmed || userData.vrodosVrTakramSkyDirectWarmed
        ) : true;
        state.takramSkyWarmupMs = usesDirectSkyCalibration ? atmosphereState && typeof atmosphereState.vrTakramSkyDirectWarmupMs === "number" ? atmosphereState.vrTakramSkyDirectWarmupMs : typeof userData.vrodosVrTakramSkyDirectWarmupMs === "number" ? userData.vrodosVrTakramSkyDirectWarmupMs : 0 : 0;
        state.takramSkyWarmupRemainingMs = usesDirectSkyCalibration ? atmosphereState && typeof atmosphereState.vrTakramSkyDirectWarmupRemainingMs === "number" ? atmosphereState.vrTakramSkyDirectWarmupRemainingMs : typeof userData.vrodosVrTakramSkyDirectWarmupRemainingMs === "number" ? userData.vrodosVrTakramSkyDirectWarmupRemainingMs : 0 : 0;
    ...
    getRuntimeFeatureState: function() {
      ...
      return {
        ...
        takram: {
          ...
          cloudWeatherRepeatX: typeof cloudDiagnostics.cloudWeatherRepeatX === "number" ? cloudDiagnostics.cloudWeatherRepeatX : null,
          cloudWeatherRepeatY: typeof cloudDiagnostics.cloudWeatherRepeatY === "number" ? cloudDiagnostics.cloudWeatherRepeatY : null,
          cloudWeatherOffsetX: typeof cloudDiagnostics.cloudWeatherOffsetX === "number" ? cloudDiagnostics.cloudWeatherOffsetX : null,
          cloudWeatherOffsetY: typeof cloudDiagnostics.cloudWeatherOffsetY === "number" ? cloudDiagnostics.cloudWeatherOffsetY : null,
          cloudMultiScatteringOctaves: typeof cloudDiagnostics.multiScatteringOctaves === "number" ? cloudDiagnostics.multiScatteringOctaves : null,
          ...
          cloudShadowFarScale: typeof cloudDiagnostics.shadowFarScale === "number" ? cloudDiagnostics.shadowFarScale : null,
          cloudShadowMaxFar: typeof cloudDiagnostics.shadowMaxFar === "number" ? cloudDiagnostics.shadowMaxFar : null,
          ...
          cloudMoonShaftsStrength: typeof cloudDiagnostics.cloudMoonShaftsStrength === "number" ? cloudDiagnostics.cloudMoonShaftsStrength : null,
          cloudMoonShaftsSourceVisibility: typeof cloudDiagnostics.cloudMoonShaftsSourceVisibility === "number" ? cloudDiagnostics.cloudMoonShaftsSourceVisibility : null,
          ...
          cloudAerialLightingMaskLayer: typeof cloudDiagnostics.aerialLightingMaskLayer === "number" ? cloudDiagnostics.aerialLightingMaskLayer : null,
          cloudAerialLightingMaskSelectedCount: typeof cloudDiagnostics.aerialLightingMaskSelectedCount === "number" ? cloudDiagnostics.aerialLightingMaskSelectedCount : null,
          ...
          cloudSunOcclusionStrength: typeof cloudDiagnostics.cloudSunOcclusionStrength === "number" ? cloudDiagnostics.cloudSunOcclusionStrength : null,
          ...
          cloudSunShadowIntensityFactor: typeof cloudDiagnostics.cloudSunShadowIntensityFactor === "number" ? cloudDiagnostics.cloudSunShadowIntensityFactor : null,
          ...
          cloudSunOcclusionTargetStrength: typeof cloudDiagnostics.cloudSunOcclusionTargetStrength === "number" ? cloudDiagnostics.cloudSunOcclusionTargetStrength : null,
          cloudMoonOcclusionEnabled: Boolean(cloudDiagnostics.cloudMoonOcclusionEnabled),
          cloudMoonOcclusionStrength: typeof cloudDiagnostics.cloudMoonOcclusionStrength === "number" ? cloudDiagnostics.cloudMoonOcclusionStrength : null,
          cloudMoonDirectFactor: typeof cloudDiagnostics.cloudMoonDirectFactor === "number" ? cloudDiagnostics.cloudMoonDirectFactor : null,
          cloudMoonIndirectFactor: typeof cloudDiagnostics.cloudMoonIndirectFactor === "number" ? cloudDiagnostics.cloudMoonIndirectFactor : null,
          cloudMoonReflectionFactor: typeof cloudDiagnostics.cloudMoonReflectionFactor === "number" ? cloudDiagnostics.cloudMoonReflectionFactor : null,
          cloudMoonShadowRadiusScale: typeof cloudDiagnostics.cloudMoonShadowRadiusScale === "number" ? cloudDiagnostics.cloudMoonShadowRadiusScale : null,
          cloudMoonShadowIntensityFactor: typeof cloudDiagnostics.cloudMoonShadowIntensityFactor === "number" ? cloudDiagnostics.cloudMoonShadowIntensityFactor : null,
          cloudMoonDiscVisibility: typeof cloudDiagnostics.cloudMoonDiscVisibility === "number" ? cloudDiagnostics.cloudMoonDiscVisibility : null,
          cloudMoonDiskOcclusion: typeof cloudDiagnostics.cloudMoonDiskOcclusion === "number" ? cloudDiagnostics.cloudMoonDiskOcclusion : null,
          cloudMoonDiskStrength: typeof cloudDiagnostics.cloudMoonDiskStrength === "number" ? cloudDiagnostics.cloudMoonDiskStrength : null,
          cloudMoonDiskUvX: typeof cloudDiagnostics.cloudMoonDiskUvX === "number" ? cloudDiagnostics.cloudMoonDiskUvX : null,
          cloudMoonDiskUvY: typeof cloudDiagnostics.cloudMoonDiskUvY === "number" ? cloudDiagnostics.cloudMoonDiskUvY : null,
          cloudMoonDiskSampleReason: cloudDiagnostics.cloudMoonDiskSampleReason || "",
          cloudMoonDiskSampleAgeMs: typeof cloudDiagnostics.cloudMoonDiskSampleAgeMs === "number" ? cloudDiagnostics.cloudMoonDiskSampleAgeMs : null,
          cloudMoonDiskSampleCount: typeof cloudDiagnostics.cloudMoonDiskSampleCount === "number" ? cloudDiagnostics.cloudMoonDiskSampleCount : null,
          cloudMoonOcclusionReason: cloudDiagnostics.cloudMoonOcclusionReason || "",
          cloudMoonStarRecoveryFactor: typeof cloudDiagnostics.cloudMoonStarRecoveryFactor === "number" ? cloudDiagnostics.cloudMoonStarRecoveryFactor : null,
          cloudMoonRecoveredStarsIntensity: typeof cloudDiagnostics.cloudMoonRecoveredStarsIntensity === "number" ? cloudDiagnostics.cloudMoonRecoveredStarsIntensity : null,
  ...
  AFRAME.registerComponent("custom-movement", {
    ...
    measureImmersiveSmoothness: function(frame, label, callback, a, b, c, d, e) {
      if (!frame || typeof callback !== "function") {
        return callback ? callback.call(this, a, b, c, d, e) : void 0;
      }
    ...
    applyConstrainedMovement: function(deltaX, deltaZ) {
      ...
      if (!currentGround) {
        currentGround = this.measureImmersiveSmoothness(smoothnessFrame, "groundSampleMs", this.sampleGroundAt, currentPosition, this.hasLastGroundHit ? this.lastGroundHit.point.y : void 0, this.sampledGroundHit);
      }
      if (!currentGround) {
