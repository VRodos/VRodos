...
${STOCHASTIC_GLSL}`).replace(
    ...
    #endif`
        ).replace("#include <map_fragment>", MAP_FRAGMENT).replace("#include <roughnessmap_fragment>", ROUGHNESS_FRAGMENT).replace("#include <metalnessmap_fragment>", METALNESS_FRAGMENT).replace("#include <normal_fragment_maps>", NORMAL_FRAGMENT).replace("#include <aomap_fragment>", AO_FRAGMENT);
      };
  ...
  (function() {
    ...
    function buildVideoControlGui(buttonEl) {
      if (typeof window.NAF === "undefined" || !window.NAF.entities || typeof lil === "undefined") {
        return;
      }
...
`;
  ...
  function vrodosApplyTextureQuality(texture, options, isColorTexture) {
    ...
    if (!texture.isVideoTexture && typeof options.maxAnisotropy === "number" && options.maxAnisotropy > 0) {
      ...
      texture.anisotropy = options.headsetProfile ? targetAnisotropy : Math.max(texture.anisotropy || 0, targetAnisotropy);
    }
  ...
  (function() {
    ...
    R.ensureSceneProbeResources = function() {
      ...
      const resolution = typeof this.settings.getSceneProbeResolution === "function" ? this.settings.getSceneProbeResolution() : 128;
    ...
    R.requestSceneProbeRefresh = function(waitForModelSettle) {
      const effectiveSource = typeof this.settings.getEffectiveReflectionSource === "function" ? this.settings.getEffectiveReflectionSource() : this.settings.getReflectionSource ? this.settings.getReflectionSource() : "none";
    ...
    R.captureSceneProbe = function(now) {
      ...
      const shouldSyncTakramHorizon = Boolean(this && this.settings.data && this.settings.data.selChoice === "0" && this.settings.data.postFXEngine === "pmndrs" && atmosphereConfig && atmosphereConfig.enabled && window.VRODOS_TAKRAM_ATMOSPHERE);
    ...
    R.getTakramSkyEnvironmentSignature = function(atmosphereConfig) {
      ...
      const moonIllumination = typeof atmosphereConfig.moonIllumination === "number" ? atmosphereConfig.moonIllumination : 1;
      const starsIntensity = typeof this.settings.getPmndrsStarsIntensity === "function" ? this.settings.getPmndrsStarsIntensity(atmosphereConfig) : 0;
      return [
    ...
    R.applyTakramSkyEnvironmentIntensity = function(atmosphereConfig, now) {
      ...
      const targetScale = typeof this.settings.getPmndrsReflectionIntensityScale === "function" ? this.settings.getPmndrsReflectionIntensityScale(atmosphereConfig, "takram-sky") : 1;
    ...
    R.updateTakramSkyEnvironment = function(now) {
      const atmosphereConfig = typeof this.settings.getPmndrsAtmosphereConfig === "function" ? this.settings.getPmndrsAtmosphereConfig() : null;
    ...
    R.applyEnvMapProfile = function() {
      const preset = typeof this.settings.getEffectiveEnvMapPreset === "function" ? this.settings.getEffectiveEnvMapPreset() : this.settings.data.envMapPreset || "none";
      const sceneObj = this.el.object3D;
      ...
      loader.load(hdrUrl, (texture) => {
        if (self.removed || !self.settings || self.settings.reflectionRuntime !== self || self._hdrEnvMapLoadId !== hdrLoadId || self._pendingHdrEnvMapPreset !== preset || self._pendingHdrEnvMapUrl !== hdrUrl) {
    ...
    function initialize(owner) {
      ...
      owner._takramSkyEnvironmentCaptureLogged = void 0;
      ...
      owner._vrodosReflectionEnvironmentIntensityScale = void 0;
      owner._vrodosReflectionEnvironmentLastUpdateMs = void 0;
    }
    const stateKeys = [
    ...
    function bind(owner, settings) {
      ...
      stateKeys.forEach((key) => Object.defineProperty(settings, key, {
        configurable: true,
        get: function() {
          return this.reflectionRuntime ? this.reflectionRuntime[key] : void 0;
        }
      }));
    }
  ...
  (function() {
    ...
    function create({
      ...
      function recordNavigationShadowRefreshSkip(self, skippedReason, eligibility) {
        ...
        shadowState._vrodosNavigationShadowRefreshLastPresentationMode = eligibility && eligibility.presentationMode ? eligibility.presentationMode : null;
  ...
  (function() {
    ...
    function create({ shadow, host }) {
      ...
      function getPmndrsCalibratedCelestialLightingProfile(config) {
        ...
        const noMoonDeepNightAmount = (1 - smoothstepNumber(-12, -8, sunElevation)) * (1 - smoothstepNumber(0.12, 0.28, illuminatedMoonVisibility));
      ...
      function getPmndrsStarsIntensity(config, self) {
        ...
        if (isPmndrsLowLightDawn(config)) {
          return mode === "on" ? getPmndrsCloudMoonStarRecovery(self, config, PMNDRS_STARS_DAWN_INTENSITY) : 0;
        }
        const sunElevation = typeof config.sunElevationDeg === "number" ? config.sunElevationDeg : null;
      ...
      function getPmndrsHorizonHelperLightConfig(self, preset, atmosphereConfig) {
        ...
        if (isPmndrsPresetTimeNight(atmosphereConfig)) {
          ...
          const moonIllumination = moonEnabled && typeof atmosphereConfig.moonIllumination === "number" ? atmosphereConfig.moonIllumination : moonEnabled ? 1 : 0;
      ...
      function getTakramShadowLightSignature(self, state) {
        ...
        const shadow2 = shadowLight && shadowLight.shadow ? shadowLight.shadow : null;
        const mapSize = shadow2 && shadow2.mapSize ? `${shadow2.mapSize.x || 0}x${shadow2.mapSize.y || 0}` : "0x0";
        const direction = shadowLight && shadowLight.sunDirection ? shadowLight.sunDirection : shadowLight && shadowLight.position ? shadowLight.position : null;
        if (!shadowLight) {
        ...
        return [
          ...
          mapSize,
          shadow2 && shadow2.map ? "map" : "no-map",
      ...
      function syncPresentedTakramLightDirections(self, config) {
        ...
        const adaptiveShadowCenter = arePmndrsDayNightCycleDynamicShadowsEnabled(self, presentedConfig) ? getAdaptiveShadowCenter(self) : null;
      ...
      function ensurePmndrsTakramHorizonLights(self, config, preset, options) {
        ...
        if (!state) {
          ...
          const moonLight2 = typeof THREE.DirectionalLight === "function" ? new THREE.DirectionalLight(PMNDRS_NIGHT_MOON_LIGHT_COLOR, PMNDRS_NIGHT_MOON_LIGHT_INTENSITY) : null;
        ...
        const cloudIndirectFactor = helperConfig.useMoonDirection ? cloudMoonOcclusion.cloudMoonIndirectFactor : cloudSunOcclusion.cloudSkyFactor;
        ...
        if (moonLight) {
          ...
          const targetMoonIntensity = moonLightEnabled && Boolean(moonDirection) ? moonIntensity * cloudMoonOcclusion.cloudMoonDirectFactor : 0;
          ...
          const moonOwnsShadow = helperConfig.useMoonDirection && moonLightEnabled && targetMoonIntensity > 1e-3;
          moonLight.castShadow = shadowEnabled && moonOwnsShadow && moonLight.visible && (!config.dayNightCycleEnabled || dynamicCycleShadows);
          ...
          if (moonLight.shadow) {
            ...
            moonLight.shadow.radius = getPmndrsDayNightShadowRadius(self) * (typeof cloudMoonOcclusion.cloudMoonShadowRadiusScale === "number" ? cloudMoonOcclusion.cloudMoonShadowRadiusScale : 1);
            ...
            if (typeof moonLight.shadow.intensity !== "undefined") {
              moonLight.shadow.intensity = typeof cloudMoonOcclusion.cloudMoonShadowIntensityFactor === "number" ? cloudMoonOcclusion.cloudMoonShadowIntensityFactor : 1;
            }
          ...
          if (self._pmndrsCloudsDiagnostics) {
            self._pmndrsCloudsDiagnostics.cloudCelestialShadowOwner = moonLight.castShadow ? "moon" : sunLight && sunLight.castShadow ? "sun" : "none";
          }
        }
        if (skyLight) {
          const targetSkyIntensity = helperConfig.fillIntensity > 0 && hasTakramSkyIrradiance ? getPmndrsTakramSkyLightIntensity(helperConfig, config) * cloudIndirectFactor : 0;
        ...
        if (fillLight) {
          const targetFillIntensity = helperConfig.fillIntensity > 0 ? getPmndrsTakramPbrFillIntensity(helperConfig, config) * cloudIndirectFactor : 0;
        ...
        if (ambientLight) {
          const cloudAmbientFactor = helperConfig.useMoonDirection ? cloudMoonOcclusion.cloudMoonIndirectFactor : typeof cloudSunOcclusion.cloudAmbientFactor === "number" ? cloudSunOcclusion.cloudAmbientFactor : 1;
  ...
  (function() {
    ...
    function create({ shadow, lighting, host }) {
      ...
      H.applyMaterialProfiles = function() {
        ...
        const cloudSunReflectionFactor = cloudSunOcclusion && typeof cloudSunOcclusion.cloudReflectionFactor === "number" ? cloudSunOcclusion.cloudReflectionFactor : 1;
        const cloudMoonReflectionFactor = cloudMoonOcclusion && typeof cloudMoonOcclusion.cloudMoonReflectionFactor === "number" ? cloudMoonOcclusion.cloudMoonReflectionFactor : 1;
  ...
  (function() {
    ...
    function create({
      ...
      function getPmndrsCloudMoonOcclusionState(self, config, smoothingMs) {
        const diagnostics = self && self._pmndrsCloudsDiagnostics ? self._pmndrsCloudsDiagnostics : null;
        const authoredCoverage = diagnostics && typeof diagnostics.authoredCoverage === "number" ? diagnostics.authoredCoverage : diagnostics && typeof diagnostics.effectiveCoverage === "number" ? diagnostics.effectiveCoverage : null;
        const diskOcclusion = diagnostics && typeof diagnostics.cloudMoonDiskOcclusion === "number" ? diagnostics.cloudMoonDiskOcclusion : 0;
        ...
        if (!self || !config || hasPmndrsDebugFlag("disablePmndrsCloudMoonInteraction", "vrodos_debug_disable_pmndrs_cloud_moon_interaction") || !(diagnostics && diagnostics.cloudsActive === true) || config.moonEnabled === false || !Number.isFinite(authoredCoverage) || moonVisibility <= 1e-3 || moonIllumination <= 1e-3 || nightFactor <= 1e-3) {
          if (self && self._pmndrsRuntimeLightSmoothValues) {
          ...
          if (!self) {
            ...
            defaultState.reason = "invalid-coverage";
          } else if (moonVisibility <= 1e-3) {
            defaultState.reason = "moon-below-horizon";
          } else if (moonIllumination <= 1e-3) {
        ...
        const currentStrength = self._pmndrsCloudMoonOcclusionState && typeof self._pmndrsCloudMoonOcclusionState.cloudMoonOcclusionStrength === "number" ? self._pmndrsCloudMoonOcclusionState.cloudMoonOcclusionStrength : factors.targetStrength;
        const strength = smoothPmndrsRuntimeLightValue(
          self,
      ...
      function getPmndrsCloudMoonStarRecovery(self, config, starsIntensity) {
        ...
        const strength = state && typeof state.cloudMoonOcclusionStrength === "number" ? clamp01(state.cloudMoonOcclusionStrength) : 0;
        if (strength <= 1e-3 || starsIntensity <= 0) {
          if (self && self._pmndrsCloudsDiagnostics) {
            self._pmndrsCloudsDiagnostics.cloudMoonStarRecoveryFactor = 0;
            self._pmndrsCloudsDiagnostics.cloudMoonRecoveredStarsIntensity = roundPmndrsCloudSunOcclusionDiagnostic(starsIntensity);
          }
          return starsIntensity;
        }
        const moonY = config && config.localMoonDirection && typeof config.localMoonDirection.y === "number" ? config.localMoonDirection.y : -1;
  ...
  (function() {
    ...
    function create({ lighting, cloud, shadow, host }) {
      ...
      function getPmndrsTakramMoonColorUrl() {
        const runtime = window.vrodos_render_runtime || {};
        const configuredUrl = window.vrodos_takram_moon_color_url || runtime.takram_moon_color_url || runtime.takramMoonColorUrl;
        if (typeof configuredUrl === "string" && configuredUrl.trim()) {
          return configuredUrl.trim();
        }
      ...
      function syncPmndrsTakramStarMoonOcclusion(material, config, state) {
        const uniforms = material && material.uniforms ? material.uniforms : null;
        const patchApplied = Boolean(uniforms && uniforms.vrodosMoonOcclusionDirection && uniforms.vrodosMoonOcclusionCosine);
        if (state) {
        ...
        if (config.moonDirection && uniforms.vrodosMoonOcclusionDirection.value && typeof uniforms.vrodosMoonOcclusionDirection.value.copy === "function") {
      ...
      function installPmndrsFallbackStarMoonOcclusion(material) {
        ...
        const originalProgramKey = typeof material.customProgramCacheKey === "function" ? material.customProgramCacheKey.bind(material) : null;
        material.onBeforeCompile = function(shader, renderer) {
...
${shader.vertexShader}`;
      ...
      function ensurePmndrsAtmosphereStars(self, config, state, vta) {
        ...
        const fallbackVisible = state.starsMesh ? false : ensurePmndrsAtmosphereStarsFallback(self, config, state, intensity);
      ...
      function updatePmndrsMoonDiagnostics(self, state, config, fallbackState) {
        ...
        const moonDefineEnabled = Boolean(defines && (typeof defines.has === "function" && defines.has("MOON") || typeof defines.has !== "function" && defines.MOON != null));
        ...
        const diagnostics = {
          ...
          cloudVisibility: cloudMoonState && typeof cloudMoonState.cloudMoonDiscVisibility === "number" ? cloudMoonState.cloudMoonDiscVisibility : 1,
          cloudOcclusionStrength: cloudMoonState && typeof cloudMoonState.cloudMoonOcclusionStrength === "number" ? cloudMoonState.cloudMoonOcclusionStrength : 0,
      ...
      function applyPmndrsMoonShaderState(self, config, state, material) {
        const uniforms = material && material.uniforms ? material.uniforms : null;
        const patchApplied = Boolean(uniforms && uniforms.vrodosMoonLightDirection && uniforms.vrodosMoonFixedToECEFMatrix && uniforms.vrodosMoonColorTexture && uniforms.vrodosMoonIllumination && uniforms.vrodosMoonHaloRadiusScale && uniforms.vrodosMoonHaloStrength && uniforms.vrodosMoonCloudVisibility);
        const cloudMoonState = self && self._pmndrsCloudMoonOcclusionState ? self._pmndrsCloudMoonOcclusionState : getPmndrsCloudMoonOcclusionState(self, config, getPmndrsRuntimeLightingSmoothingMs(config));
        const cloudVisibility = cloudMoonState && typeof cloudMoonState.cloudMoonDiscVisibility === "number" ? cloudMoonState.cloudMoonDiscVisibility : 1;
  ...
  (function() {
    ...
    function formatPmndrsCloudLightingForLog(self) {
      ...
      const cloudMoonState = [
        ...
        diagnostics.cloudMoonShaftsActive ? `shafts${formatPmndrsNumberForLog(diagnostics.cloudMoonShaftsStrength, 3)}` : `shafts-skip-${diagnostics.cloudMoonShaftsSkippedReason || "inactive"}`
      ].join(":");
      return {
        clouds: cloudsState,
    ...
    function logPmndrsCloudSunOcclusionDiagnostic(self, diagnostics) {
      ...
      log.call(console, `[VRodos] PMNDRS cloud lighting: clouds=${cloudLightingLog.clouds}, cloudSun=${cloudLightingLog.cloudSun}, cloudMoon=${cloudLightingLog.cloudMoon}`);
    }
    ...
    H.updatePmndrsHorizonSun = function(options) {
      ...
      const fixedHeadsetSun = atmosphereRuntime && typeof this.isVrRuntimeHeadsetProfile === "function" && this.isVrRuntimeHeadsetProfile() && typeof this.isPmndrsDayNightCycleActive === "function" && !this.isPmndrsDayNightCycleActive() && this._pmndrsSunSpriteActive !== true && this._pmndrsCloudSunDiskSpriteActive !== true && !(this._pmndrsCloudsDiagnostics && this._pmndrsCloudsDiagnostics.cloudsActive);
      ...
      const immersivePresentation = fixedHeadsetSun && typeof this.isDirectVrPresentationActive === "function" ? this.isDirectVrPresentationActive() : false;
      const staticFrame = atmosphereRuntime && atmosphereRuntime.staticHeadsetSunFrame;
      if (options && options.poseOnly && fixedHeadsetSun && staticFrame && staticFrame.data === this.data && staticFrame.state === atmosphereState && staticFrame.yaw === presentedYaw && staticFrame.immersive === immersivePresentation && atmosphereState && atmosphereState.ready && atmosphereState.skyMesh && atmosphereState.skyMesh.visible && atmosphereState.skyMesh.parent === this.el.object3D) {
