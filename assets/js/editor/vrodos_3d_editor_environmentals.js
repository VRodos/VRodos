...
const VRODOS_DIRECTOR_GROUND_GUIDE = Object.freeze({
    updateIntervalMs: 50,
...
function vrodosDirectorGroundGuideObjectExcluded(object) {
    ...
    while (current) {
        ...
        if (VRODOS_DIRECTOR_GROUND_GUIDE_EXCLUDED_NAMES.has(current.name)) {
            return true;
        }
...
function vrodosDirectorGroundGuideObjectVisible(object) {
    let current = object || null;
...
function vrodosEnvironmentResolveBaseUrl(pluginPath, localizedKey, fallbackRelative) {
    ...
    if (pluginBaseUrl) {
        return vrodosEnvironmentJoinUrl(pluginBaseUrl, fallbackRelative);
    }
