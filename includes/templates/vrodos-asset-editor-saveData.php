...
function vrodos_create_asset_3DFilesExtra_frontend($assetNewId, $projectId, $assetCatId){
    ...
    if ($_POST['glbFileInput']) {
        ...
        if (strlen($_POST['glbFileInput']) > 0) {
            $glbFile_id = VRodos_Upload_Manager::upload_asset_text(null, 'glb_' . $assetNewId . '_' . $assetCatId, $assetNewId,
                $_FILES, 0, $projectId);
...
function vrodos_create_asset_addAudio_frontend($asset_newID){
    ...
    $attachment_audio_id = VRodos_Upload_Manager::upload_img_vid_aud( $asset_audioForm, $asset_newID);
...
function vrodos_create_asset_addVideo_frontend($asset_newID) {
    ...
    $attachment_video_id = VRodos_Upload_Manager::upload_img_vid_aud( $asset_videoForm, $asset_newID);
